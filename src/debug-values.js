...
async function debugValues() {
  try {
    ...
    console.log(
      `💰 [DEBUG] VALOR TOTAL CALCULADO MANUALMENTE: R$ ${totalValue.toFixed(2)}`
    );
    ...
    console.log(
      `💰 [DEBUG] VALOR DO DATAMANAGER: R$ ${dataManagerValue.toFixed(2)}`
    );
    ...
    console.log(
      `🟰 Valores iguais: ${totalValue === dataManagerValue ? "✅ SIM" : "❌ NÃO"}`
    );
  } catch (error) {

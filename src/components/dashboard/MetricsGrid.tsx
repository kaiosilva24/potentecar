          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                console.log(
                  `✅ [DEBUG] Encontrados ${stockItems.length} itens de estoque:`
                );
                console.table(
                  stockItems.map((item) => ({
                    Nome: item.item_name,
                ...
                const totalDataManager =
                  await dataManager.calculateTotalStockValue();

                console.log("📊 [DEBUG] COMPARAÇÃO DE VALORES:");
                console.log(`🔢 Cálculo Manual: R$ ${totalManual.toFixed(2)}`);
                console.log(
                  `🔧 DataManager: R$ ${totalDataManager.toFixed(2)}`
                );
                console.log(
                  `🎯 Valores iguais: ${totalManual === totalDataManager ? "✅ SIM" : "❌ NÃO"}`
                );

                if (totalManual !== totalDataManager) {
                  console.warn(
                    "⚠️ [DEBUG] VALORES DIFERENTES! Investigando..."
                  );
                  ...
                  for (let i = 0; i < localStorage.length; i++) {
                    ...
                    if (
                      key &&
                      (key.includes("stock") || key.includes("value"))
                    ) {
                      console.log(
                        `🗎 localStorage[${key}]:`,

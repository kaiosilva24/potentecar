...
const Sidebar = ({
  ...
  const { lowStockItems, lowStockCount, isLoading } =
  ...
  const [debtsTooltipPosition, setDebtsTooltipPosition] = useState({
    x: 0,
    y: 0,
  });
  ...
  const handleLowStockMouseEnter = (event: React.MouseEvent) => {
    ...
    if (spaceOnRight >= tooltipWidth + 20) {
      ...
      x = Math.max(
        10,
        Math.min(rect.left, window.innerWidth - tooltipWidth - 10)
      );
    }
  ...
  return (
    <div
      className={`${isCollapsed ? "w-[60px]" : "w-[280px]"} h-screen bg-factory-900/90 backdrop-blur-md border-r border-tire-700/30 flex flex-col glass-morphism transition-all duration-300 relative overflow-visible`}
    >
      {/* Botão de Toggle */}
      <Button
      ...
      <div
        className={`${isCollapsed ? "p-2" : "pt-6 px-6 pb-2"} flex flex-col items-center`}
      >
        {!isCollapsed && (
          <>
            <img
            ...
            />
            <p className="text-sm text-tire-400 text-center">
      ...
      <ScrollArea className="flex-1 px-0.5 pt-6 max-h-[calc(100vh-200px)]">
        ...
        {!isCollapsed && (
          <div className="space-y-3 px-2">
            ...
            >
              <span
                className={`h-2 w-2 rounded-full ${lowStockCount > 0 ? "bg-red-400 animate-pulse" : "bg-tire-600"}`}
              ></span>
              {isLoading
                ? "Verificando..."
            ...
            >
              <span
                className={`h-2 w-2 rounded-full ${overdueCount > 0 ? "bg-red-500 animate-pulse" : "bg-tire-600"}`}
              ></span>
              {debtsLoading
                ? "Verificando..."
      ...
      <div
        className={`${isCollapsed ? "p-2" : "p-2"} mt-auto border-t border-tire-700/30`}
      >
        {defaultBottomItems.map((item) => (
          <Button
            ...
            onClick={() =>
              item.label === "Logout" ? signOut() : onItemClick(item.label)
            }
            title={
              isCollapsed
                ? item.label === "Settings"

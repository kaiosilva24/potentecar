...
const AuthPerformanceDebug: React.FC = () => {
  ...
  const refreshPerformanceData = () => {
    ...
    const avgSessionCheck =
      AuthPerformanceMonitor.getAverageTime("session_check");
    const signInSuccess = AuthPerformanceMonitor.getSuccessRate("sign_in");
    const sessionSuccess =
      AuthPerformanceMonitor.getSuccessRate("session_check");
  ...
  const runPerformanceTest = async (
    ...
    try {
      ...
      console.error(
        `❌ [AuthDebug] ${testName} falhou após ${duration}ms:`,
        error
      );
    } finally {
      setIsLoading(false);
  ...
  const testLazyLoadSecondaryData = () => {
    runPerformanceTest(
      "Carregamento Lazy de Dados Secundários",
      lazyLoadSecondaryData
    );
  };
        ...
        <CardContent className="space-y-4">
          ...
          {performanceData && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="p-3 bg-factory-700/30 rounded border border-tire-600/20">
                ...
                <div
                  className={`text-lg font-bold ${getPerformanceColor(performanceData.avgSignIn, "time")}`}
                >
                  {performanceData.avgSignIn > 0
                    ? `${performanceData.avgSignIn}ms`
                    : "N/A"}
                </div>
              </div>
              ...
              <div className="p-3 bg-factory-700/30 rounded border border-tire-600/20">
                ...
                <div
                  className={`text-lg font-bold ${getPerformanceColor(performanceData.avgSessionCheck, "time")}`}
                >
                  {performanceData.avgSessionCheck > 0
                    ? `${performanceData.avgSessionCheck}ms`
                    : "N/A"}
                </div>
              </div>
              ...
              <div className="p-3 bg-factory-700/30 rounded border border-tire-600/20">
                ...
                <div
                  className={`text-lg font-bold ${getPerformanceColor(performanceData.signInSuccess, "rate")}`}
                >
              ...
              <div className="p-3 bg-factory-700/30 rounded border border-tire-600/20">
                ...
                <div
                  className={`text-lg font-bold ${getPerformanceColor(performanceData.sessionSuccess, "rate")}`}
                >
          ...
          <div className="p-3 bg-factory-700/20 rounded border border-tire-600/20">
            ...
            <ul className="text-sm text-tire-300 space-y-1">
              <li>
                • <strong>Timeout de Login:</strong> 10 segundos (vs. padrão 15
                minutos)
              </li>
              <li>
                • <strong>Cache de Sessão:</strong> 5 minutos de duração
              </li>
              <li>
                • <strong>Verificação Rápida:</strong> 3 segundos de timeout
              </li>
              <li>
                • <strong>Pré-carregamento:</strong> Dados críticos em paralelo
              </li>
              <li>
                • <strong>Lazy Loading:</strong> Dados secundários em background
              </li>
              <li>
                • <strong>Fallback:</strong> localStorage como backup
              </li>
            </ul>
          </div>
        </CardContent>

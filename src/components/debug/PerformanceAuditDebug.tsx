...
const PerformanceAuditDebug: React.FC = () => {
  ...
  const [liveMetrics, setLiveMetrics] = useState<{ metrics: PerformanceMetric[]; queries: QueryAnalysis[] }>({ metrics: [], queries: [] });

  const runFullAudit = async () => {
    setIsRunning(true);
    try {
      console.log('🔍 [PerformanceAuditDebug] Executando auditoria completa...');
  ...
  const getPerformanceIcon = (duration: number) => {
    if (duration < 1000) return <CheckCircle className="h-4 w-4 text-green-400" />;
      ...
      <Card className="bg-factory-800/50 border-tire-600/30">
        <CardHeader>
          <CardTitle className="text-tire-200 text-lg flex items-center gap-2">
            ...
            <Badge className={isEnabled ? "bg-green-600" : "bg-red-600"}>
              {isEnabled ? <Play className="h-3 w-3 mr-1" /> : <Pause className="h-3 w-3 mr-1" />}
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              <Search className={`h-4 w-4 mr-2 ${isRunning ? 'animate-spin' : ''}`} />
              Executar Auditoria
            </Button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="p-3 bg-factory-700/30 rounded border border-tire-600/20">
              ...
              <div className="text-lg font-bold text-red-400">
                {liveMetrics.metrics.filter(m => m.duration && m.duration > 2000).length}
              </div>
            </div>
          ...
          {auditResults && (
            <div className="space-y-4">
              <h4 className="text-tire-200 font-medium">Resultados da Auditoria:</h4>
              ...
              <div className="p-4 bg-factory-700/20 rounded border border-tire-600/20">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <div className="text-tire-200 font-medium">{auditResults.summary.totalOperations}</div>
                  </div>
                  <div>
                    <span className="text-tire-400">Queries:</span>
                    <div className="text-tire-200 font-medium">{auditResults.summary.totalQueries}</div>
                  </div>
                  <div>
                    <span className="text-tire-400">Tempo Médio Op:</span>
                    <div className="text-tire-200 font-medium">{formatDuration(auditResults.summary.averageOperationTime)}</div>
                  </div>
                  <div>
                    <span className="text-tire-400">Tempo Médio Query:</span>
                    <div className="text-tire-200 font-medium">{formatDuration(auditResults.summary.averageQueryTime)}</div>
                  </div>
                </div>
              </div>
              ...
              {auditResults.slowOperations.length > 0 && (
                <div className="space-y-2">
                  <h5 className="text-tire-200 font-medium">Operações Lentas (Top 5):</h5>
                  <div className="space-y-1">
                    {auditResults.slowOperations.slice(0, 5).map((op: PerformanceMetric, index: number) => (
                      <div 
                        key={index}
                      ...
                      >
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-tire-200 text-sm">{op.operation}</span>
                        </div>
                        <div className={`text-sm font-medium ${getPerformanceColor(op.duration || 0)}`}>
              ...
              {auditResults.slowQueries.length > 0 && (
                <div className="space-y-2">
                  <h5 className="text-tire-200 font-medium">Queries Lentas (Top 5):</h5>
                  <div className="space-y-1">
                    {auditResults.slowQueries.slice(0, 5).map((query: QueryAnalysis, index: number) => (
                      <div 
                        key={index}
                        className="flex items-center justify-between p-2 bg-factory-700/30 rounded border border-tire-600/20"
                      >
                        <div className="flex items-center gap-2">
                          <Database className="h-4 w-4 text-purple-400" />
                          <span className="text-tire-200 text-sm">{query.table}.{query.operation}</span>
                          {query.rowCount && (
                            <Badge variant="outline" className="text-xs border-tire-600 text-tire-400">
                              {query.rowCount} rows
                            </Badge>
                          )}
                        </div>
                        <div className={`text-sm font-medium ${getPerformanceColor(query.duration)}`}>
          ...
          <div className="p-3 bg-factory-700/20 rounded border border-tire-600/20">
            ...
            <ul className="text-sm text-tire-300 space-y-1">
              <li>• <strong>Executar Auditoria:</strong> Analisa performance completa do sistema</li>
              <li>• <strong>Métricas em Tempo Real:</strong> Atualizam automaticamente a cada 5s</li>
              <li>• <strong>Pausar/Ativar:</strong> Controla se o auditor está coletando dados</li>
              <li>• <strong>Cores:</strong> Verde (&lt;1s), Amarelo (1-3s), Vermelho (&gt;3s)</li>
            </ul>
          </div>
        </CardContent>
      </Card>

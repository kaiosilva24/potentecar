...
import { Settings, CheckCircle, XCircle, RefreshCw, Trash2, Eye } from "lucide-react";
...
const TireCostDebug: React.FC = () => {
  ...
  const handleInitialize = () => {
    ...
    try {
      ...
      console.log("✅ [TireCostDebug] Custos padrão inicializados com sucesso!");
    } catch (error) {
    ...
    <Card className="bg-factory-800/50 border-tire-600/30">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="text-sm text-tire-300">
          <p><strong>Total de tamanhos:</strong> {DEFAULT_TIRE_COSTS.length}</p>
          <p><strong>Status:</strong> {isInitialized ? 'Dados padrão disponíveis' : 'Dados padrão não encontrados'}</p>
        </div>
              ...
              {debugInfo.map((info, index) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <Badge className={getStatusColor(info.hasData, info.isDefault)}>
                      {getStatusText(info.hasData, info.isDefault)}
                    </Badge>
                    <span className="text-tire-200 text-sm">{info.productName}</span>
                  </div>
                  <div className="text-right text-sm">
                    {info.hasData ? (
                      <>
                        <div className="text-green-400">R$ {info.costPerTire}</div>
                        <div className="text-tire-400 text-xs">{info.source}</div>
                      </>
                    ) : (
        ...
        <div className="p-3 bg-factory-700/20 rounded border border-tire-600/20">
          ...
          <ul className="text-sm text-tire-300 space-y-1">
            <li>• <strong>Inicializar:</strong> Cria dados padrão para todos os tamanhos de pneus</li>
            <li>• <strong>Limpar:</strong> Remove apenas os dados padrão (preserva dados personalizados)</li>
            <li>• <strong>Inspecionar:</strong> Mostra o status atual de cada tamanho de pneu</li>
          </ul>
        </div>

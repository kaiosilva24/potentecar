...
const TireCostInitializationDebug: React.FC = () => {
  ...
  const checkInitializationStatus = () => {
    console.log(
      "🔍 [TireCostInitializationDebug] Verificando status de inicialização..."
    );

    const status: Record<string, any> = {};
    const allTireAnalysisKeys = Object.keys(localStorage).filter((key) =>
      key.includes("tireAnalysis")
    );
    ...
    DEFAULT_TIRE_COSTS.forEach((defaultCost) => {
      ...
      if (existingData) {
        try {
          ...
          console.error(
            `Erro ao parsear dados para ${defaultCost.productName}:`,
            error
          );
        }
      }
    ...
    status["_summary"] = {
      ...
      initializedCount:
        Object.values(status).filter(
          (s: any) => s.initialized && s.productKey !== "_summary"
        ).length - 1, // -1 para excluir _averageCost
      ...
      <CardContent className="space-y-4">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                {problematicStatus.initialized ? "Sim" : "Não"}
              </div>
              <div>
                <strong>É Padrão:</strong>{" "}
                {problematicStatus.isDefault ? "Sim" : "Não"}
              </div>
              <div>
                <strong>Custo:</strong> R${" "}
                {problematicStatus.costPerTire.toFixed(2)}
              </div>
            </div>
        ...
        <details className="mt-4">
          <summary className="cursor-pointer font-medium">
            ...
            {
              Object.keys(initializationStatus).filter(
                (k) => !k.startsWith("_")
              ).length
            }
            )

  ...
  useEffect(() => {
    ...
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === "dashboard_tireCostValue_unified") {
        console.log(
          "🔄 [ProductStock] Detectada mudança no custo por pneu - atualizando"
        );
        updateCostPerTire();
      }
    };
    ...
    const handleTireCostUpdate = (event: CustomEvent) => {
      ...
      if (averageCostPerTire > 0) {
        ...
        console.log(
          "🔄 [ProductStock] Custo atualizado via evento:",
          averageCostPerTire
        );
      }
    };
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      if (relevantMutation) {
        console.log(
          "🔍 [ProductStock] Mudança relevante detectada no DOM - atualizando custo"
        );
        updateCostPerTire();
      }
    });
    ...
    const intervalId = setInterval(() => {
      console.log(
        "⏰ [ProductStock] Verificação periódica - atualizando custo atual"
      );
      updateCostPerTire();

...
const PresumedProfitChart = ({
  ...
  const getFilteredSales = () => {
    ...
    console.log(
      "🔍 [PresumedProfitChart] Debug filtros (APENAS PRODUTOS FINAIS):",
      {
        dateFilter,
        totalCashFlowEntries: cashFlowEntries.length,
        allSalesEntries: cashFlowEntries.filter(
    ...
    switch (dateFilter) {
      case "all":
        ...
        console.log(
          "✅ [PresumedProfitChart] Usando filtro 'all' - mostrando todos os dados"
        );
        break;
      case "custom":
  ...
  const extractProductInfoFromSale = (description: string) => {
    try {
      ...
      if (!description.includes("TIPO_PRODUTO: final")) {
        console.log(
          "🚫 [PresumedProfitChart] Ignorando venda não-final:",
      ...
      if (quantityMatch && productNameMatch) {
        ...
        console.log(
          "❌ [PresumedProfitChart] Falha na extração - campos obrigatórios não encontrados:",
          {
      ...
      console.error(
        "❌ [PresumedProfitChart] Erro ao extrair informações do produto:",
        error
      );
    }
    return null;
  ...
  const profitData = useMemo(() => {
    console.log(
      "🚀 [PresumedProfitChart] Iniciando cálculo de dados de lucro..."
    );

    const salesEntries = getFilteredSales();
    console.log(
      "📊 [PresumedProfitChart] Vendas filtradas para processamento:",
      {
    ...
    salesEntries.forEach((entry, index) => {
      console.log(
        `🔍 [PresumedProfitChart] Processando venda ${index + 1}/${salesEntries.length}:`,
        {
          amount: entry.amount,
          date: entry.transaction_date,
      ...
      if (productInfo && productInfo.productName) {
        ...
        console.log(
          "⚠️ [PresumedProfitChart] Falha na extração - tentando fallback..."
        );
        if (entry.description) {
          ...
          if (match) {
            ...
            console.log(
              `🔄 [PresumedProfitChart] Produto identificado via fallback: ${productName}`
            );
          } else {
            console.log(
              "❌ [PresumedProfitChart] Fallback também falhou - usando nome genérico"
            );
          }
        }
      }
      ...
      console.log(
        `📊 [PresumedProfitChart] Somando quantidade para ${productName}:`,
        {
    ...
    console.log(
      "🎯 [PresumedProfitChart] Resultado final do cálculo de lucro:",
      {

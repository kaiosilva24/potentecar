...
const RawMaterialStock = ({
  ...
  const handleStockOperation = (operation: "add" | "remove") => {
    if (selectedMaterial && quantity && parseFloat(quantity) > 0) {
      try {
        const selectedMaterialData = activeMaterials.find(
        ...
        console.log(
          `📝 [RawMaterialStock] Resultado do onStockUpdate:`,
          result
        );
        ...
        setTimeout(() => {
          console.log(
            `⏰ [RawMaterialStock] Após 3s - Eventos recebidos: ${eventCounterRef.current}`
          );
          setNotification((prev) => ({ ...prev, show: false }));
        }, 3000);
      } catch (error) {
        console.error(
          "❌ [RawMaterialStock] Erro na operação de estoque:",
          error
        );
        setNotification({
          show: true,
      ...
      console.warn(
        `⚠️ [RawMaterialStock] Operação não executada - dados insuficientes:`,
        {
          selectedMaterial,
          quantity,
  ...
  const calculateMaterialMetrics = () => {
    ...
    if (hasItems && Math.random() < 0.1) {
      // Log apenas 10% das vezes para reduzir spam
      console.log(
        "🔍 [RawMaterialStock] Calculando:",
  ...
  useEffect(() => {
    const handleStockItemsUpdate = (event: CustomEvent) => {
      ...
      console.log(
        `📡 [RawMaterialStock] Evento stockItemsUpdated #${eventCounterRef.current} recebido:`,
        {
          timestamp: new Date().toISOString(),
      ...
      setForceUpdate((prev) => {
        ...
        console.log(
          `📊 [RawMaterialStock] forceUpdate: ${prev} -> ${newValue}`
        );
        return newValue;
      });
      ...
      if (event.detail.item && event.detail.item.item_type === "material") {
        ...
        console.log(
          `🔍 [RawMaterialStock] Procurando material com ID: ${item.item_id}`
        );
        const material = activeMaterialsRef.current.find(
          (m) => m.id === item.item_id
        );

        if (material) {
          console.log(
            `✅ [RawMaterialStock] Material encontrado:`,
            material.name
          );
          console.log(
            `🔄 [RawMaterialStock] Atualização: ${material.name} -> ${item.quantity} ${material.unit}`
          );
          setNotification({
            show: true,
            type: "success",
          ...
          console.warn(
            `⚠️ [RawMaterialStock] Material não encontrado com ID: ${item.item_id}`
          );
          console.log(
            `📋 [RawMaterialStock] Materiais disponíveis:`,
            activeMaterialsRef.current.map((m) => ({ id: m.id, name: m.name }))
          );
        }
      } else {
        console.log(
          `🔕 [RawMaterialStock] Evento não é de material ou item não encontrado`
        );
      }
    };

    console.log(
      "🔔 [RawMaterialStock] Registrando listener para stockItemsUpdated (uma única vez)"
    );
    ...
    return () => {
      console.log(
        "🚫 [RawMaterialStock] Removendo listener para stockItemsUpdated"
      );
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-6 bg-factory-900/90 backdrop-blur-md rounded-2xl border border-tire-700/30">
      ...
      {notification.show && (
        <div
          className={`mb-6 p-4 rounded-lg border flex items-center gap-3 transition-all duration-300 ${
            ...
            <div className="space-y-2">
                    ...
                    if (e.key === "ArrowDown") {
                      ...
                      if (filteredMaterialsForDropdown.length > 0) {
                        const newIndex =
                          ...
                          filteredMaterialsForDropdown.length - 1
                        ...
                        setTimeout(() => {
                          const dropdown = document.querySelector(
                            ".absolute.z-50.mt-1.bg-factory-800"
                          );
                      ...
                      if (
                        selectedMaterialIndex >= 0 &&
                        filteredMaterialsForDropdown[selectedMaterialIndex]
                      ) {
              ...
              {showMaterialDropdown &&
                filteredMaterialsForDropdown.length > 0 && (
                  <div
                    className="absolute z-50 mt-1 bg-factory-800 border border-tire-600/30 rounded-lg shadow-lg overflow-y-auto"
                    style={{ width: "300px", maxHeight: "240px" }}
                  >
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && quantity) {
                    const unitPriceInput = document.querySelector(
                      'input[placeholder="Digite o preço por unidade"]'
                    ) as HTMLInputElement;
                    if (unitPriceInput) {

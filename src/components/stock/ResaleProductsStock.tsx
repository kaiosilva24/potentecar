...
const ResaleProductsStock = ({
  isLoading = false,
}: ResaleProductsStockProps) => {
  ...
  const [filterStatus, setFilterStatus] = useState<"all" | "low" | "normal">(
    "all"
  );
  ...
  const handleStockUpdate = async () => {
    ...
    const product = resaleProductsWithStock.find(
      ...
      if (product.stockId) {
        ...
        if (stockOperation === "add") {
          ...
          if (priceValue > 0) {
            ...
            newUnitCost =
              newQuantity > 0 ? newTotalValue / newQuantity : priceValue;
          } else {
          ...
          if (priceValue > 0) {
            ...
            console.log(
              `➖ [ResaleProductsStock] REMOÇÃO COM VALOR (recalcula custo médio):`,
              {
                currentQuantity,
                currentTotalValue,
            ...
            console.log(
              `➖ [ResaleProductsStock] REMOÇÃO SEM VALOR (usa custo médio):`,
              {
                currentQuantity,
                quantityRemoved: quantityValue,
                newQuantity,
        ...
        if (valueDifference > 0) {
          ...
          console.log(
            `💰 [ResaleProductsStock] Adicionando R$ ${valueDifference.toFixed(2)} ao baseline`
          );
          ...
          console.log(
            `💰 [ResaleProductsStock] Subtraindo R$ ${Math.abs(valueDifference).toFixed(2)} do baseline`
          );
        ...
        console.log(
          "📡 [ResaleProductsStock] Disparando evento de sincronização..."
        );
        ...
        console.log(
          "🔄 [ResaleProductsStock] Forçando recarregamento dos dados de estoque..."
        );
        window.dispatchEvent(
          new CustomEvent("forceStockItemsReload", {
            detail: {
        ...
        setTimeout(() => {
          console.log(
            "⚡ [ResaleProductsStock] Disparando evento de atualização imediata..."
          );
          window.dispatchEvent(
            new CustomEvent("forceChartsRefresh", {
              detail: {
                source: "ResaleProductsStock",
                productId: product.id,
                timestamp: new Date().toISOString(),
        ...
        const newUnitCost =
          priceValue > 0 ? priceValue : product.purchase_price || 0;
        ...
        // Disparar evento para sincronização em tempo real com StockCharts
        console.log(
          "📡 [ResaleProductsStock] Disparando evento de sincronização para novo estoque..."
        );
        window.dispatchEvent(
          new CustomEvent("resaleStockUpdated", {
            detail: {
        ...
        // Forçar recarregamento dos dados de estoque para sincronização
        console.log(
          "🔄 [ResaleProductsStock] Forçando recarregamento dos dados de estoque para novo item..."
        );
        window.dispatchEvent(
          new CustomEvent("forceStockItemsReload", {
            detail: {
        ...
        setTimeout(() => {
          console.log(
            "⚡ [ResaleProductsStock] Disparando evento de atualização imediata para novo item..."
          );
          window.dispatchEvent(
            new CustomEvent("forceChartsRefresh", {
              detail: {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      console.log(
        "💰 [ResaleProductsStock] Sincronizando valor total e quantidade total:",
        {
      ...
      try {
        ...
        console.log(
          "💾 [ResaleProductsStock] Valor salvo no localStorage:",
          stockData
        );
      } catch (error) {
        console.warn(
          "⚠️ [ResaleProductsStock] Erro ao salvar no localStorage:",
          error
        );
      }
      ...
      try {
        ...
        const success =
          await dataManager.saveResaleProductTotalQuantity(totalQuantity);
        if (success) {
          console.log(
            `✅ [ResaleProductsStock] Quantidade total salva no Supabase: ${totalQuantity}`
          );
          ...
          console.log(
            "📡 [ResaleProductsStock] Evento resaleProductTotalQuantityUpdated disparado:",
            {
              quantity: totalQuantity,
              source: "ResaleProductsStock",
            }
        ...
        console.warn(
          "⚠️ [ResaleProductsStock] Erro ao salvar quantidade total no Supabase:",
          error
        );
      }
      ...
      console.log(
        "📡 [ResaleProductsStock] Evento resaleTotalStockUpdated disparado:",
        {
    ...
    <div className="w-full max-w-7xl mx-auto p-6 bg-factory-900/90 backdrop-blur-md rounded-2xl border border-tire-700/30">
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6 p-4 bg-factory-800/30 rounded-lg border border-tire-600/20">
        ...
        <Select
          value={filterStatus}
          onValueChange={(value) =>
            setFilterStatus(value as "all" | "low" | "normal")
          }
        >
        ...
        <Dialog open={showStockDialog} onOpenChange={setShowStockDialog}>
          <DialogTrigger asChild>
            <Button
              style={{ backgroundColor: "#49de80", color: "#1d1e21" }}
              onMouseEnter={(e) =>
                (e.currentTarget.style.backgroundColor = "#3bc46e")
              }
              onMouseLeave={(e) =>
                (e.currentTarget.style.backgroundColor = "#49de80")
              }
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setStockOperation(value as "add" | "remove")
                    }
                  >
                    <SelectTrigger className="bg-factory-700/50 border-tire-600/30 text-white">
                      <SelectValue />
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-tire-600/30">
                    ...
                    <TableHead className="text-tire-300 text-center">
                      Quantidade
                    </TableHead>
                    <TableHead className="text-tire-300 text-center">
                      Nível Mín.
                ...
                <TableBody>
                  {filteredProducts.map((product) => (
                    <TableRow
                      key={product.id}
                      className="border-tire-600/30 hover:bg-tire-700/20"
                    >
                      ...
                      <TableCell className="text-right text-tire-300">
                        {product.averageUnitValue > 0
                          ? formatCurrency(product.averageUnitValue)
                          : "-"}
                      </TableCell>

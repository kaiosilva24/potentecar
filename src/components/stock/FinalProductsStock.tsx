...
const FinalProductsStock: React.FC<FinalProductsStockProps> = ({
  isLoading = false,
}) => {
  const {
    stockItems,
  ...
  const [selectedProductForMinLevel, setSelectedProductForMinLevel] =
    useState("");
    ...
    try {
      ...
      if (savedAnalysis) {
        try {
          ...
          console.warn(
            `⚠️ [FinalProductsStock] Erro ao parsear análise específica para "${productName}":`,
            parseError
          );
        }
      }
      ...
      if (synchronizedData) {
        try {
          ...
          if (data.value && data.value > 0) {
            console.log(
              `📊 [FinalProductsStock] Usando custo médio para "${productName}": R$ ${data.value.toFixed(2)}`
            );
          ...
          console.warn(
            `⚠️ [FinalProductsStock] Erro ao parsear custo médio:`,
            parseError
          );
        }
      }
      ...
      console.warn(
        `⚠️ [FinalProductsStock] Nenhum custo encontrado para "${productName}", usando R$ 0.00`
      );
      return 0;
    } catch (error) {
      console.error(
        `❌ [FinalProductsStock] Erro ao buscar custo específico para "${productName}":`,
        error
      );
      return 0;
    }
  ...
  useEffect(() => {
    console.log(
      "🔌 [FinalProductsStock] Iniciando configuração de listeners..."
    );

    const handleStockChartsUpdate = (event: CustomEvent) => {
      console.log(
        "📡 [FinalProductsStock] Recebendo atualização em tempo real do StockCharts:",
        event.detail
      );
      ...
      setRealTimeUpdates((prev) => {
        ...
        console.log(
          "🔄 [FinalProductsStock] Atualizando estado realTimeUpdates:",
          newState
        );
        return newState;
      });
      ...
      if (event.detail.finalProductsData) {
        console.log(
          "🔄 [FinalProductsStock] Processando dados atualizados em tempo real:",
          event.detail.finalProductsData
        );
    ...
    const handleTireCostUpdate = (event: CustomEvent) => {
      console.log(
        "💰 [FinalProductsStock] Recebendo atualização de custo do TireCostManager:",
        event.detail
      );
      ...
      setRealTimeUpdates((prev) => {
        ...
        console.log(
          "🔄 [FinalProductsStock] Atualizando estado com custos do TireCostManager:",
          newState
        );
        return newState;
      });
      ...
      console.log(
        "🔄 [FinalProductsStock] Forçando recálculo com novos custos por pneu..."
      );
    };

    const handleRealtimeUpdate = (event: CustomEvent) => {
      console.log(
        "⚡ [FinalProductsStock] Atualização Supabase Realtime recebida:",
        event.detail
      );
      ...
      setRealTimeUpdates((prev) => {
        ...
        console.log(
          "🔄 [FinalProductsStock] Atualizando estado com Supabase:",
          newState
        );
        return newState;
      });
    ...
    console.log(
      "🌐 [FinalProductsStock] Window disponível, adicionando listeners..."
    );

    // Adicionar listeners para eventos customizados
    window.addEventListener(
      "stockChartsDataUpdated",
      handleStockChartsUpdate as EventListener
    );
    window.addEventListener(
      "stockChartsRealTimeUpdate",
      handleRealtimeUpdate as EventListener
    );

    console.log(
      "🔔 [FinalProductsStock] Listeners de sincronização em tempo real configurados"
    );
    ...
    setTimeout(() => {
      console.log(
        "🧪 [FinalProductsStock] Testando listeners com evento de teste..."
      );
    ...
    return () => {
      ...
      console.log(
        "🔕 [FinalProductsStock] Listeners de sincronização removidos"
      );
    };
  ...
  useEffect(() => {
    console.log(
      "💰 [FinalProductsStock] Monitorando mudanças nos custos em tempo real..."
    );

    const handleStorageChange = (event: StorageEvent) => {
      // Monitorar mudanças no localStorage relacionadas aos custos
      if (
        event.key === "dashboard_averageCostPerTire" ||
        (event.key && event.key.startsWith("tireAnalysis_"))
      ) {
        console.log(
          "🔄 [FinalProductsStock] Mudança detectada no localStorage:",
          {
            key: event.key,
  ...
  const syncTotalValueWithDashboard = async (totalValue: number) => {
    ...
    try {
      setIsSyncing(true);
      console.log(
        `📊 [FinalProductsStock] Sincronizando valor total: R$ ${totalValue.toFixed(2)}`
      );

      // Salvar no Supabase via dataManager
      const success =
        await dataManager.saveFinalProductStockBalance(totalValue);

      if (success) {
        console.log(
          `✅ [FinalProductsStock] Valor salvo com sucesso no Supabase: R$ ${totalValue.toFixed(2)}`
        );
        ...
        console.log(
          `📡 [FinalProductsStock] Evento 'finalProductStockUpdated' disparado para dashboard`
        );
      } else {
        console.error(
          `❌ [FinalProductsStock] Erro ao salvar valor no Supabase`
        );
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const currentTotalValue = productAnalysis.reduce(
      (total, product) => total + (product?.totalValue || 0),
      0
    );
    ...
    if (
      ...
      Math.abs(currentTotalValue - lastSyncedValue) > 0.01
    ) {
      console.log(
        `🔄 [FinalProductsStock] Valor total mudou de R$ ${(lastSyncedValue || 0).toFixed(2)} para R$ ${currentTotalValue.toFixed(2)}`
      );
  ...
  useEffect(() => {
    if (
      ...
      const initialTotalValue = productAnalysis.reduce(
        (total, product) => total + (product?.totalValue || 0),
        0
      );
      console.log(
        `🚀 [FinalProductsStock] Inicializando sincronização com valor inicial: R$ ${initialTotalValue.toFixed(2)}`
      );
  ...
  useEffect(() => {
    const handleStockChartsDataUpdate = (event: CustomEvent) => {
      ...
      console.log(
        `📡 [FinalProductsStock] Evento 'stockChartsDataUpdated' recebido de ${source || "Unknown"}:`
      );
      console.log(
      ...
      if (finalProductsData && finalProductsData.length > 0) {
        console.log(
          "🔄 [FinalProductsStock] Forçando re-análise com dados do StockCharts..."
        );
    ...
    const handleForceStockRecalculation = (event: CustomEvent) => {
      ...
      console.log(
        `🔄 [FinalProductsStock] Evento 'forceStockRecalculation' recebido de ${source || "Unknown"}:`
      );
      console.log(
        `  - Timestamp: ${timestamp ? new Date(timestamp).toLocaleString() : "N/A"}`
      );
      ...
      console.log(
        `💰 [FinalProductsStock] Valor atual calculado: R$ ${currentTotalValue.toFixed(2)}`
      );
    ...
    const handleTireCostUpdate = (event: CustomEvent) => {
      const { averageCostPerTire, specificAnalyses, timestamp, source } =
        event.detail || {};
      console.log(
        `💰 [FinalProductsStock] Evento 'tireCostUpdated' recebido de ${source || "Unknown"}:`
      );
      console.log(
        `  - Timestamp: ${timestamp ? new Date(timestamp).toLocaleString() : "N/A"}`
      );
      console.log(
        `  - Custo médio por pneu: R$ ${averageCostPerTire ? averageCostPerTire.toFixed(2) : "0.00"}`
      );
      console.log(
        `  - Análises específicas: ${specificAnalyses?.length || 0} produtos`
      );
      ...
      console.log(
        "🔄 [FinalProductsStock] Forçando recalculação com novos custos sincronizados..."
      );
    };

    console.log(
      "🎯 [FinalProductsStock] Registrando listeners para eventos do StockCharts e TireCostManager"
    );

    // Adicionar listeners para os eventos customizados
    window.addEventListener(
      "stockChartsDataUpdated",
      handleStockChartsDataUpdate as EventListener
    );
    window.addEventListener(
      "forceStockRecalculation",
      handleForceStockRecalculation as EventListener
    );
    window.addEventListener(
      "tireCostUpdated",
    ...
    return () => {
      console.log(
        "🚫 [FinalProductsStock] Removendo listeners para eventos do StockCharts e TireCostManager"
      );
      window.removeEventListener(
        "stockChartsDataUpdated",
        handleStockChartsDataUpdate as EventListener
  ...
  const handleSaveQuantity = async (productId: string) => {
    console.log(
      `🚀 [FinalProductsStock] handleSaveQuantity CHAMADA para produto: ${productId}`
    );
    const product = productAnalysis.find((p) => p.productId === productId);
    if (!product) {
      console.log(
        `❌ [FinalProductsStock] Produto não encontrado: ${productId}`
      );
      return;
    }
    console.log(
      `✅ [FinalProductsStock] Produto encontrado: ${product.productName}, quantidade editável: ${product.editableQuantity}`
    );
    ...
    try {
      ...
      if (stockItem) {
        ...
        console.log(
          `✅ Quantidade atualizada para ${product.productName}: ${product.editableQuantity} unidades, Valor total: ${formatCurrency(newTotalValue)}`
        );

        // NOVO: Disparar evento para atualizar StockDashboard em tempo real
        console.log(
          `🔍 [FinalProductsStock] Preparando cálculo do valor total após atualização...`
        );
        console.log(
          `📊 [FinalProductsStock] Estado atual do productAnalysis:`,
        ...
        const totalValue = productAnalysis.reduce((sum, p) => {
          if (p.productId === productId) {
            console.log(
              `🔄 [FinalProductsStock] Produto atualizado: ${p.productName} - Valor antigo: R$ ${p.totalValue.toFixed(2)}, Valor novo: R$ ${newTotalValue.toFixed(2)}`
            );
            return sum + newTotalValue;
          }
          console.log(
            `✅ [FinalProductsStock] Produto inalterado: ${p.productName} - Valor: R$ ${p.totalValue.toFixed(2)}`
          );
          return sum + p.totalValue;
        }, 0);

        console.log(
          `💰 [FinalProductsStock] Valor total calculado: R$ ${totalValue.toFixed(2)}`
        );
        ...
        const eventDetail = {
          ...
          lowStockProducts: productAnalysis.filter(
            (p) => p.stockLevel === "low"
          ).length,
          timestamp: Date.now(),
        ...
        console.log(
          `📡 [FinalProductsStock] Disparando evento 'finalProductStockUpdated':`
        );
        ...
        console.log(
          `  - Produto atualizado: ${eventDetail.updatedProduct.productName} (${eventDetail.updatedProduct.newQuantity} unidades)`
        );

        // Verificar se window está disponível
        if (typeof window === "undefined") {
          console.error(
            `❌ [FinalProductsStock] Window não está disponível para disparar eventos!`
          );
          return;
        }

        console.log(
          `🌐 [FinalProductsStock] Window disponível, disparando eventos...`
        );

        try {
          window.dispatchEvent(
            new CustomEvent("finalProductStockUpdated", {
              detail: eventDetail,
            })
          );
          console.log(
            `✅ [FinalProductsStock] Evento 'finalProductStockUpdated' disparado com sucesso!`
          );
          ...
          console.log(
            `✅ [FinalProductsStock] Evento 'stockItemsUpdated' disparado com sucesso!`
          );
        } catch (eventError) {
          console.error(
            `❌ [FinalProductsStock] Erro ao disparar eventos:`,
            eventError
          );
        }
      }
  ...
  const calculateGrandTotal = () => {
    return filteredProductAnalysis.reduce(
      (total, product) => total + product.totalValue,
      0
    );
  };
  ...
  const handleSetMinLevel = async () => {
    ...
    const stockItem = stockItems.find(
      (item) =>
        item.item_id === selectedProductForMinLevel &&
    ...
    try {
      if (stockItem) {
        ...
        const product = products.find(
          (p) => p.id === selectedProductForMinLevel
        );
        if (product) {
  ...
  const lowStockCount = productAnalysis.filter(
    (product) => product.stockLevel === "low"
  ).length;
    ...
    <div className="w-full max-w-7xl mx-auto p-6 bg-factory-900/90 backdrop-blur-md rounded-2xl border border-tire-700/30">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          ...
          <Dialog
            open={showMinLevelDialog}
            onOpenChange={setShowMinLevelDialog}
          >
            ...
            <DialogContent className="bg-factory-800 border-tire-600/30 text-white">
              ...
              <div className="space-y-4">
                <div>
                  <Label>Produto</Label>
                  <Select
                    value={selectedProductForMinLevel}
                    onValueChange={setSelectedProductForMinLevel}
                  >
      ...
      {productAnalysis.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          <div
            className="border border-neon-green rounded-lg p-4"
            style={{ backgroundColor: "#173329" }}
          >
          ...
          filteredProductAnalysis.map((product) => (
            ...
            >
              ...
              <div className="bg-factory-700/30 rounded-lg p-4 border border-tire-600/20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div>
                    <Label className="text-tire-400 text-sm mb-1 block">
                      Quantidade Atual
                    </Label>
                    {product.isEditing ? (
                      <Input
                  ...
                  <div>
                    ...
                    <div className="bg-factory-600/30 border border-tire-600/20 rounded-md px-3 py-2 h-10 flex items-center">
                      <span className="text-neon-green font-medium">
                        {formatCurrency(
                          product.isEditing
                            ? product.editableQuantity * product.costPerTire
                            : product.totalValue
                        )}
                      </span>
                    </div>

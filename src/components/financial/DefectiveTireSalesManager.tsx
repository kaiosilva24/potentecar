...
const DefectiveTireSalesManager = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log(
      "📅 [DefectiveTireSalesManager] Ajuste de data para registro:",
      {
  ...
  const filteredSales = defectiveTireSales.filter((sale) => {
    ...
    console.log("🔍 [DefectiveTireSalesManager] Filtro de data aplicado:", {
      ...
      allSales: defectiveTireSales.map((s) => ({
    ...
    switch (dateFilterType) {
      case "custom":
        if (customStartDate && customEndDate) {
          ...
          matchesDate =
            sale.sale_date >= adjustedStartDate &&
            sale.sale_date <= adjustedEndDate;
          console.log(
            "📅 [DefectiveTireSalesManager] Filtro personalizado aplicado:",
            {
                ...
                <p className="text-2xl font-bold text-yellow-400">
                  {formatCurrency(
                    filteredSales.length > 0
                      ? Math.max(
                          ...filteredSales.map((sale) => sale.sale_value)
                        )
                      : 0
                  )}
                </p>
              ...
              <div>
                ...
                <p className="text-lg font-bold text-cyan-400 truncate">
                  {(() => {
                    ...
                    const tireCount = filteredSales.reduce(
                      (acc, sale) => {
                        acc[sale.tire_name] =
                          (acc[sale.tire_name] || 0) + sale.quantity;
                        return acc;
                      },

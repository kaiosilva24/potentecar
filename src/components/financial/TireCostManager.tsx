...
const TireCostManager = ({
  ...
  const averageAnalysis = useMemo(() => {
    ...
    return {
      ...
      averageCostPerTire:
        Math.round(
          (totals.totalProduced > 0
            ? tireAnalysis.reduce(
                (sum, tire) => sum + tire.costPerTire * tire.totalProduced,
                0
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        value: (() => {
          ...
          const materialCost =
            costOptions.divideByProduction && averageAnalysis.totalProduced > 0
              ? tireAnalysis.reduce(
                  (sum, tire) =>
                    sum + tire.recipeCostPerTire * tire.totalProduced,
                  0
          ...
          const cashFlowCost = costOptions.includeCashFlowExpenses
            ? costOptions.divideByProduction &&
          ...
          const lossCost = costOptions.includeProductionLosses
            ? costOptions.divideByProduction &&
              averageAnalysis.totalProduced > 0
              ? tireAnalysis.reduce(
                  (sum, tire) => sum + (tire.lossDetails?.totalLossValue || 0),
                  0
                ) / averageAnalysis.totalProduced
              : tireAnalysis.reduce(
                  (sum, tire) => sum + (tire.lossDetails?.totalLossValue || 0),
          ...
          const defectiveCost = costOptions.includeDefectiveTireSales
            ? costOptions.divideByProduction &&
              averageAnalysis.totalProduced > 0
              ? -(
                  defectiveTireSales.reduce(
                    (total, sale) => total + sale.sale_value,
                    0
                  ) / averageAnalysis.totalProduced
                )
          ...
          const warrantyCost = costOptions.includeWarrantyValues
            ? costOptions.divideByProduction &&
              averageAnalysis.totalProduced > 0
              ? tireAnalysis.reduce(
                  (sum, tire) =>
                    sum + (tire.warrantyDetails?.totalWarrantyValue || 0),
                  0
                ) / averageAnalysis.totalProduced
              : tireAnalysis.reduce(
                  (sum, tire) =>
    ...
    const saveToSupabase = async () => {
      try {
        // Calcular o custo correto (soma dos componentes)
        const correctCost = (() => {
          if (tireAnalysis.length === 0) return 0;
        ...
        const success = await dataManager.saveAverageTireCost(correctCost);
        if (success) {
          console.log(
          ...
          averageCostPerTire: (() => {
            ...
            const lossCost = costOptions.includeProductionLosses
              ? costOptions.divideByProduction &&
                ...
                ? tireAnalysis.reduce(
                    (sum, tire) =>
                      sum + (tire.lossDetails?.totalLossValue || 0),
                    0
                  ) / averageAnalysis.totalProduced
                : tireAnalysis.reduce(
                    (sum, tire) =>
                ...
                <p className="text-xl font-bold text-neon-orange">
                  {formatCurrency(
                    (() => {
                      ...
                      // Usar soma dos componentes como valor correto
                      const materialCost =
                        costOptions.divideByProduction &&
                        averageAnalysis.totalProduced > 0
                          ? tireAnalysis.reduce(
                              (sum, tire) =>
                                sum +
                                tire.recipeCostPerTire * tire.totalProduced,
                              0
                            ) / averageAnalysis.totalProduced
                          : tireAnalysis.reduce(
                              (sum, tire) =>
                      ...
                      const warrantyCost = costOptions.includeWarrantyValues
                        ? costOptions.divideByProduction &&
                          ...
                          ? tireAnalysis.reduce(
                              (sum, tire) =>
                                sum +
                                (tire.warrantyDetails?.totalWarrantyValue || 0),
                              0
                            ) / averageAnalysis.totalProduced
                          : tireAnalysis.reduce(
                              (sum, tire) =>
                              ...
                              <span className="text-neon-orange font-bold">
                                {formatCurrency(
                                  (() => {
                                    ...
                                    // Usar soma dos componentes como valor principal
                                    const materialCost =
                                      costOptions.divideByProduction &&
                                      averageAnalysis.totalProduced > 0
                                        ? tireAnalysis.reduce(
                              ...
                              <span className="text-white font-medium text-lg">
                                {formatCurrency(
                                  (() => {
                                    ...
                                    return costOptions.divideByProduction &&
                                      averageAnalysis.totalProduced > 0
                                      ? totalMaterialCost /
                                          averageAnalysis.totalProduced
                                      : totalMaterialCost;
                                    ...
                                    (() => {
                                      ...
                                      const totalLossValue =
                                        tireAnalysis.reduce(
                                          (sum, tire) =>
                                            sum +
                                            (tire.lossDetails?.totalLossValue ||
                                              0),
                                          0
                                        );
                                      return costOptions.divideByProduction &&

...
const DebtManager = ({
  ...
  const getDebtPaymentsFromCashFlow = useMemo(() => {
    ...
    const debtPayments = cashFlowEntries.filter(
      (entry) =>
        entry.type === "expense" &&
    ...
    console.log(
      "💳 [DEBT INTEGRATION] Pagamentos de dívida encontrados no fluxo de caixa:",
      debtPayments
    );
  ...
  const totals = useMemo(() => {
    ...
    const totalPaidFromDebts = debts.reduce(
      (sum, debt) =>
        sum +
        (typeof debt.paid_amount === "number"
          ? debt.paid_amount
    ...
    const totalPaidFromCashFlow = getDebtPaymentsFromCashFlow.reduce(
  ...
  useEffect(() => {
    const syncDebtPayments = async () => {
      ...
      console.log(
        "🔄 [DEBT SYNC] Iniciando sincronização automática de pagamentos..."
      );
      ...
      for (const payment of getDebtPaymentsFromCashFlow) {
        ...
        if (!debt) {
          console.log(
            `⚠️ [DEBT SYNC] Dívida ${debtId} não encontrada para pagamento de ${payment.amount}`
          );
          continue;
        }
        ...
        if (expectedRemainingAmount < 0) {
          console.log(
            `⚠️ [DEBT SYNC] Pagamento de ${payment.amount} excede valor restante da dívida ${debt.description}`
          );
          continue;
        }
        ...
        try {
          ...
          console.log(
            `✅ [DEBT SYNC] Dívida ${debt.description} atualizada com pagamento de ${payment.amount}`
          );
        } catch (error) {
          console.error(
            `❌ [DEBT SYNC] Erro ao sincronizar pagamento para dívida ${debtId}:`,
            error
          );
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(
        "❌ [DebtManager] Detalhes do erro:",
        JSON.stringify(error, null, 2)
      );
      alert(
        `Erro ao cadastrar dívida: ${error.message || "Erro desconhecido"}`
      );
    }
  };
  ...
  const handleDelete = async (debtId: string) => {
    ...
    if (
      debt &&
      confirm(`Tem certeza que deseja excluir a dívida: ${debt.description}?`)
    ) {
      try {
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      <div
                        className={`flex items-center gap-1 ${getStatusColor(debt.status)}`}
                      >

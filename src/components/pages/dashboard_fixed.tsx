...
const EmpresarialProfitChart = ({
  ...
  const loadBusinessProfitChartData = async () => {
    try {
      ...
      const historicalData = await dataManager.loadBusinessProfitHistory(days);
      ...
      for (let i = days - 1; i >= 0; i--) {
        ...
        if (realDataPoint) {
          ...
          chartData.push({
            date: dateStr,
            displayDate: dateStr,
            profit: realDataPoint.businessProfit,
            businessValue: realDataPoint.businessValue,
            baseline: realDataPoint.baseline,
          });
          console.log(
            "📊 [EmpresarialProfitChart] Usando dados reais para",
            dateStr,
            ":",
            realDataPoint.businessProfit
          );
        } else {
          // Sem dados para esta data - mostrar 0
          chartData.push({
            date: dateStr,
            displayDate: dateStr,
            profit: 0,
          ...
          console.log(
            "📊 [EmpresarialProfitChart] Sem dados para",
            dateStr,
            "- usando 0"
          );
        }
  ...
  const saveCurrentProfitToHistory = async () => {
    if (empresarialValue !== null && businessBaseline !== null) {
      const currentProfit = empresarialValue - businessBaseline;
      const success = await dataManager.saveBusinessProfitHistory(
        empresarialValue,
        currentProfit,
        businessBaseline
      );
      if (success) {
        console.log(
          "✅ [EmpresarialProfitChart] Histórico de lucro salvo com sucesso!"
        );
...
const MainDashboard = ({ isLoading = false }: { isLoading?: boolean }) => {
  ...
  useEffect(() => {
    console.log("🚀 [Dashboard] Iniciando carregamento progressivo...");

    // Fase 1: Dados críticos carregados
    if (!cashFlowLoading && !debtsLoading) {
      console.log("✅ [Dashboard] Fase 1 completa - dados críticos carregados");
      setLoadingPhase("secondary");

      // Iniciar carregamento da fase 2 após um pequeno delay
      setTimeout(() => {
        setShowSecondaryData(true);
        console.log("🔄 [Dashboard] Iniciando fase 2 - dados secundários");
      }, 100);
    }
  }, [cashFlowLoading, debtsLoading]);

  // Fase 2: Dados secundários carregados
  useEffect(() => {
    if (
      showSecondaryData &&
      ...
      console.log(
        "✅ [Dashboard] Fase 2 completa - dados secundários carregados"
      );
      setLoadingPhase("complete");

      // Iniciar carregamento da fase 3 após delay maior
      setTimeout(() => {
        setLoadTertiaryData(true);
        console.log("🔄 [Dashboard] Iniciando fase 3 - dados terciários");
      }, 500);
    }
  }, [
    showSecondaryData,
  ...
  useEffect(() => {
    if (loadingPhase === "secondary") {
      const timer = setTimeout(() => {
        setChartsInitialized(true);
        console.log("📊 [Dashboard] Gráficos inicializados");
      }, 200); // Delay reduzido
  ...
  // Contador de vendas
  const salesCount = cashFlowEntries.filter(
  ...
  const resaleProductStockValue = useMemo(() => {
    ...
    console.log(
      `📊 [MainDashboard] Valor total de produtos de revenda: R$ ${value.toFixed(2)}`
    );
    return value;
      ...
      try {
        ...
        if (localValue && localValue !== "null" && localValue !== "0") {
          ...
          if (!isNaN(parsed) && parsed > 0) {
            console.log(
              `⚡ [Dashboard] Valor inicial carregado do localStorage: R$ ${parsed.toFixed(2)}`
            );
            return parsed;
          }
        }
        ...
        console.log(
          "⚠️ [Dashboard] Erro ao carregar valor inicial do localStorage"
        );
      }
      return null;
    }
  ...
  const updateResaleProfitWithDebounce = (
    newProfit: number,
    source: string
  ) => {
    const now = Date.now();
    ...
    console.log(
      `🔄 [Dashboard] Tentativa de atualização de ${source}: R$ ${newProfit.toFixed(2)}`
    );
    console.log(
      `🕰️ [Dashboard] Tempo desde última atualização: ${timeSinceLastUpdate}ms`
    );

    // Se já há uma atualização pendente com o mesmo valor, ignorar
    if (pendingUpdate !== null && Math.abs(pendingUpdate - newProfit) < 0.01) {
      console.log(
        `⚠️ [Dashboard] Atualização ignorada - valor já pendente: R$ ${pendingUpdate.toFixed(2)}`
      );
      return;
    }
    ...
    if (
      ...
      console.log(
        `✅ [Dashboard] Valor já atualizado: R$ ${averageResaleProfit.toFixed(2)}`
      );
      return;
    }
    ...
    if (
      ...
      console.log(
        `⛔ [Dashboard] Rejeitando atualização para R$ 0.00 de ${source} - mantendo valor atual: R$ ${averageResaleProfit.toFixed(2)}`
      );
      return;
    }
    ...
    if (timeSinceLastUpdate < 300) {
      console.log(
        `🔄 [Dashboard] Debounce ativo - agendando atualização em 300ms`
      );
      setPendingUpdate(newProfit);

      setTimeout(() => {
        console.log(
          `⏰ [Dashboard] Executando atualização agendada: R$ ${newProfit.toFixed(2)}`
        );
      ...
      console.log(
        `⚡ [Dashboard] Atualização imediata: R$ ${newProfit.toFixed(2)}`
      );
      setAverageResaleProfit(newProfit);
      setIsLoadingResaleProfit(false);
  ...
  useEffect(() => {
    if (!stockItemsLoading && stockItems.length >= 0) {
      const timeoutId = setTimeout(async () => {
        console.log(
          "🔍 [Dashboard] Calculando saldo de matéria-prima automaticamente..."
        );
        console.log(`📊 [Dashboard] Total de stockItems: ${stockItems.length}`);
        ...
        console.log(
          `🏭 [Dashboard] Matérias-primas encontradas: ${materialItems.length}`
        );
        ...
        const unitaryMaterialQuantity = materialItems.reduce((total, item) => {
          ...
          if (itemUnit === "un" && item.quantity > 0) {
            console.log(
              `📦 [Dashboard] Material unitário encontrado: ${item.item_name} - Qtd: ${item.quantity}`
            );
        ...
        console.log(
          `📦 [Dashboard] Quantidade unitária de matéria-prima: ${unitaryMaterialQuantity}`
        );
        ...
        materialItems.forEach((item) => {
          ...
          if (itemValue > 0) {
            console.log(
              `  - ${item.item_name}: R$ ${itemValue.toFixed(2)} (qtd: ${item.quantity}, custo: ${item.unit_cost})`
            );
          }
        });

        console.log(
          `💰 [Dashboard] Saldo calculado: R$ ${newBalance.toFixed(2)}`
        );
        console.log(
          `💰 [Dashboard] Saldo atual no estado: R$ ${(rawMaterialStockBalance || 0).toFixed(2)}`
        );
        ...
        if (
          ...
          console.log(
            `🔄 [Dashboard] Atualizando saldo de matéria-prima: R$ ${newBalance.toFixed(2)}`
          );
          ...
          if (success) {
            console.log(
              `✅ [Dashboard] Saldo salvo com sucesso no Supabase: R$ ${newBalance.toFixed(2)}`
            );
          ...
          console.log(
            `✅ [Dashboard] Saldo já atualizado, não há necessidade de alterar`
          );
        }

        // Salvar quantidade unitária de matéria-prima no Supabase também
        try {
          const success = await dataManager.saveRawMaterialUnitaryQuantity(
            unitaryMaterialQuantity
          );
          if (success) {
            console.log(
              `✅ [Dashboard] Quantidade unitária de matéria-prima salva no Supabase: ${unitaryMaterialQuantity}`
            );
            console.log(
              `📦 [Dashboard] Card "Matéria Prima Unitária" sincronizado com valor: ${unitaryMaterialQuantity}`
            );
          ...
          console.warn(
            "⚠️ [Dashboard] Erro ao salvar quantidade unitária de matéria-prima:",
            error
          );
        }
        ...
        console.log(
          "🔍 [Dashboard] Calculando quantidade total de produtos finais..."
        );
        ...
        console.log(
          `🏭 [Dashboard] Produtos finais encontrados: ${finalProductItems.length}`
        );
        ...
        const totalFinalProductQuantity = finalProductItems.reduce(
          (total, item) => {
            ...
            console.log(
              `🏭 [Dashboard] Produto Final: ${item.item_name} - Qtd: ${quantity}`
            );
        ...
        console.log(
          `🏭 [Dashboard] Quantidade total de produtos finais calculada: ${totalFinalProductQuantity}`
        );
        ...
        if (
          ...
          console.log(
            `🔄 [Dashboard] Atualizando quantidade total de produtos finais: ${totalFinalProductQuantity}`
          );

          // Salvar no Supabase
          const success = await dataManager.saveFinalProductTotalQuantity(
            totalFinalProductQuantity
          );
          if (success) {
            console.log(
              `✅ [Dashboard] Quantidade total salva com sucesso no Supabase: ${totalFinalProductQuantity}`
            );
          ...
          console.log(
            `✅ [Dashboard] Quantidade total já atualizada: ${totalFinalProductQuantity}`
          );
        }

        // ===== CALCULAR QUANTIDADE TOTAL DE PRODUTOS REVENDA =====
        console.log(
          "🔍 [Dashboard] Calculando quantidade total de produtos revenda..."
        );
        ...
        console.log(
          `🛍️ [Dashboard] Produtos revenda encontrados: ${resaleProductItems.length}`
        );
        ...
        const totalResaleProductQuantity = resaleProductItems.reduce(
          (total, item) => {
            const quantity = Number(item.quantity) || 0;
            console.log(
              `🛍️ [Dashboard] Produto Revenda: ${item.item_name} - Qtd: ${quantity}`
            );
            return total + quantity;
          },
          0
        ...
        console.log(
          `🛍️ [Dashboard] Quantidade total de produtos revenda calculada: ${totalResaleProductQuantity}`
        );
        ...
        if (
          ...
          console.log(
            `🔄 [Dashboard] Atualizando quantidade total de produtos revenda: ${totalResaleProductQuantity}`
          );

          // Salvar no Supabase
          const success = await dataManager.saveResaleProductTotalQuantity(
            totalResaleProductQuantity
          );
          if (success) {
            console.log(
              `✅ [Dashboard] Quantidade total de revenda salva com sucesso no Supabase: ${totalResaleProductQuantity}`
            );
          ...
          console.log(
            `✅ [Dashboard] Quantidade total de revenda já atualizada: ${totalResaleProductQuantity}`
          );
        }
  ...
  useEffect(() => {
    if (!cashFlowLoading && cashFlowEntries.length >= 0) {
      const timeoutId = setTimeout(async () => {
        console.log(
          "🔍 [Dashboard] Calculando saldo de caixa automaticamente..."
        );
        console.log(
          `📊 [Dashboard] Total de entradas no cashFlow: ${cashFlowEntries.length}`
        );
        ...
        console.log(
          `💰 [Dashboard] Saldo atual no estado: R$ ${(cashBalanceState || 0).toFixed(2)}`
        );
        ...
        if (
          ...
          console.log(
            `🔄 [Dashboard] Atualizando saldo de caixa: R$ ${newBalance.toFixed(2)}`
          );
  ...
  useEffect(() => {
    const initializeBalances = () => {
      console.log(
        "🚀 [Dashboard] Inicializando sincronização de todos os saldos..."
      );

      try {
        // Usar saldo de caixa já calculado
        console.log(
          `💰 [Dashboard] Saldo de caixa: R$ ${cashBalance.toFixed(2)}`
        );
        ...
        console.log(
          `🏭 [Dashboard] Saldo de matéria-prima: R$ ${materialStockValue.toFixed(2)}`
        );
        ...
        const finalProductStockValue = stockItems
          ...
          .reduce((total, item) => total + (item.total_value || 0), 0);
        console.log(
          `📦 [Dashboard] Saldo de produtos finais: R$ ${finalProductStockValue.toFixed(2)}`
        );
        ...
        console.log(
          `🛒 [Dashboard] Saldo de produtos de revenda: R$ ${resaleProductStockValue.toFixed(2)}`
        );
        setResaleProductStockBalance(resaleProductStockValue);
        setIsLoadingResaleProductStock(false);

        console.log(
          "✅ [Dashboard] Todos os saldos sincronizados com sucesso!"
        );
        console.log("🔍 [Dashboard] RESUMO DOS SALDOS INICIALIZADOS:");
        console.log(`  - cashBalanceState: R$ ${cashBalance.toFixed(2)}`);
        console.log(
          `  - rawMaterialStockBalance: R$ ${materialStockValue.toFixed(2)}`
        );
        console.log(
          `  - finalProductStockBalance: R$ ${finalProductStockValue.toFixed(2)}`
        );
        console.log(
          `  - resaleProductStockBalance: R$ ${resaleProductStockValue.toFixed(2)}`
        );
        console.log(
          `  - SOMA TOTAL (sem dívidas): R$ ${(cashBalance + materialStockValue + finalProductStockValue + resaleProductStockValue).toFixed(2)}`
        );
      } catch (error) {
  ...
  useEffect(() => {
    const calculateEmpresarialValue = () => {
      ...
      if (
        ...
        console.log(
          "💼 [Dashboard] Calculando Valor Empresarial (SOMA DINÂMICA COM DÍVIDAS):"
        );
        console.log(`  - Saldo Caixa: R$ ${cashBalanceState.toFixed(2)}`);
        console.log(
          `  - Saldo Matéria-Prima: R$ ${rawMaterialStockBalance.toFixed(2)}`
        );
        console.log(
          `  - Saldo Produtos Finais: R$ ${finalProductStockBalance.toFixed(2)}`
        );
        console.log(
          `  - Saldo Produtos Revenda: R$ ${resaleProductStockBalance.toFixed(2)}`
        );
        console.log("💳 [Dashboard] DETALHAMENTO DAS DÍVIDAS:");
        console.log(`  - Total de dívidas encontradas: ${debts.length}`);
        debts.forEach((debt, index) => {
          console.log(
            `    ${index + 1}. ${debt.description}: remaining_amount = R$ ${(typeof debt.remaining_amount === "number" ? debt.remaining_amount : parseFloat(debt.remaining_amount) || 0).toFixed(2)}`
          );
        });
        console.log(
          `  - SALDO DEVEDOR TOTAL: R$ ${totalDebtRemaining.toFixed(2)}`
        );
        console.log(
          `  - VALOR EMPRESARIAL CALCULADO: R$ ${calculatedBusinessValue.toFixed(2)}`
        );
        console.log(
          `  - FÓRMULA DETALHADA: ${cashBalanceState.toFixed(2)} + ${rawMaterialStockBalance.toFixed(2)} + ${finalProductStockBalance.toFixed(2)} + ${resaleProductStockBalance.toFixed(2)} - ${totalDebtRemaining.toFixed(2)} = ${calculatedBusinessValue.toFixed(2)}`
        );
        console.log(
          `  - VERIFICAÇÃO: ${(cashBalanceState + rawMaterialStockBalance + finalProductStockBalance + resaleProductStockBalance - totalDebtRemaining).toFixed(2)}`
        );
        ...
        if (Math.abs(calculatedBusinessValue - manualCalculation) > 0.01) {
          console.error(
            "❌ [Dashboard] ERRO: Inconsistência no cálculo do valor empresarial!"
          );
          console.error(
            `  - calculatedBusinessValue: R$ ${calculatedBusinessValue.toFixed(2)}`
          );
          console.error(
            `  - manualCalculation: R$ ${manualCalculation.toFixed(2)}`
          );
          console.error(
            `  - Diferença: R$ ${Math.abs(calculatedBusinessValue - manualCalculation).toFixed(2)}`
          );
        }

        console.log(
          `🎯 [Dashboard] VALOR EMPRESARIAL FINAL DEFINIDO: R$ ${calculatedBusinessValue.toFixed(2)}`
        );
        ...
        dataManager
          .saveBusinessValue(calculatedBusinessValue)
          .catch((error) => {
            console.error(
              "❌ [Dashboard] Erro ao salvar valor empresarial no banco:",
              error
            );
          });
      } else {
        console.log(
          "⏳ [Dashboard] Aguardando todos os valores para calcular Valor Empresarial..."
        );
        console.log(
          `  - Saldo Caixa: ${cashBalanceState !== null ? `R$ ${cashBalanceState.toFixed(2)}` : "LOADING"}`
        );
        console.log(
          `  - Saldo Matéria-Prima: ${rawMaterialStockBalance !== null ? `R$ ${rawMaterialStockBalance.toFixed(2)}` : "LOADING"}`
        );
        console.log(
          `  - Saldo Produtos Finais: ${finalProductStockBalance !== null ? `R$ ${finalProductStockBalance.toFixed(2)}` : "LOADING"}`
        );
        console.log(
          `  - Saldo Produtos Revenda: ${resaleProductStockBalance !== null ? `R$ ${resaleProductStockBalance.toFixed(2)}` : "LOADING"}`
        );
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Gráfico de Lucro Empresarial - Carregamento Prioritário */}
      <EmpresarialProfitChart
        cashFlowEntries={cashFlowEntries}
        isLoading={isCriticalDataLoading}
      ...
      {loadTertiaryData && (
        <>
          ...
          <div className="hidden">
            <FinalProductsStock isLoading={isSecondaryDataLoading} />
          </div>
          ...
          <div className="hidden">
            <ResaleProductsStock isLoading={isSecondaryDataLoading} />
          </div>
        </>
      )}
      ...
      <div className="hidden">
        {(() => {
          const isLoadingValue =
            ...
            warrantyEntriesLoading;
          console.log(
            "💼 [Dashboard] Renderizando PresumedProfitManager para calcular Valor Empresarial:",
            {

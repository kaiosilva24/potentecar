  ...
  const calculateUnitMaterialCost = (entry: ProductionEntry) => {
    ...
    entry.materials_consumed.forEach((material) => {
      ...
      const stockItem = stockItems.find(
        (item) =>
          item.item_id === material.material_id && item.item_type === "material"
      );
    ...
    return entry.quantity_produced > 0
      ? totalCost / entry.quantity_produced
      : 0;
  };

  // Advanced filtering logic
  const filteredEntries = productionEntries.filter((entry) => {
    // Excluir entradas de garantia (quantity_produced = 0)
    ...
    switch (dateFilterType) {
      case "today":
        ...
        console.log(
          "🗓️ [DailyProduction] Filtro 'hoje' com workaround aplicado:",
          {
      ...
      case "custom":
        if (customStartDate && customEndDate) {
          ...
          matchesDate =
            entryDate >= adjustedStartDate && entryDate <= adjustedEndDate;
                          ...
                          <div className="mt-3 pt-2 border-t border-tire-600/20">
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-neon-green font-bold text-lg">
                                {formatCurrency(
                                  calculateUnitMaterialCost(entry)
                                )}
                              </span>
                            </div>
                            <p className="text-xs text-tire-400 mt-1">
                              Custo dos materiais para produzir 1 unidade do
                              produto
                            </p>
                          </div>
                        </div>

...
export async function setupSystemSettingsTable(): Promise<boolean> {
  try {
    ...
    if (error) {
      ...
      console.log(
        "📝 [SetupSupabase] 1. Acesse: https://supabase.com/dashboard"
      );
      console.log(
        "📝 [SetupSupabase] 2. Selecione seu projeto: yrxixhpnepuccpvungnc"
      );
      console.log("📝 [SetupSupabase] 3. Vá para SQL Editor");
      console.log(
        "📝 [SetupSupabase] 4. Execute o arquivo: create_system_settings_table.sql"
      );
    ...
    console.log(
      "✅ [SetupSupabase] Tabela system_settings existe e está acessível!"
    );
    return true;
  } catch (error) {
    console.error("❌ [SetupSupabase] Erro ao verificar tabela:", error);
    console.log(
      "📝 [SetupSupabase] Execute o SQL manualmente no Supabase Dashboard"
    );
    return false;
  }
}
...
export async function checkSystemSettingsTable(): Promise<boolean> {
  try {
    ...
    if (error) {
      console.warn(
        "⚠️ [SetupSupabase] Tabela system_settings não existe ou não acessível:",
        error.message
      );
      return false;
    }
...
export async function setupTireCostHistoryTable(): Promise<boolean> {
  try {
    ...
    if (error) {
      ...
      console.log(
        "📝 [SetupSupabase] INSTRUÇÕES PARA CRIAR TABELA TIRE_COST_HISTORY:"
      );
      console.log(
        "📝 [SetupSupabase] 1. Acesse: https://supabase.com/dashboard"
      );
...
CREATE INDEX IF NOT EXISTS idx_tire_cost_history_date ON tire_cost_history(date);
    ...
    console.log(
      "✅ [SetupSupabase] Tabela tire_cost_history existe e está acessível!"
    );
    return true;
  } catch (error) {
    console.error(
      "❌ [SetupSupabase] Erro ao verificar tabela tire_cost_history:",
      error
    );
    console.log(
      "📝 [SetupSupabase] Execute o SQL manualmente no Supabase Dashboard"
...
export async function autoSetupSupabase(): Promise<boolean> {
  try {
    ...
    if (!systemSettingsOk) {
      console.log(
        "❌ [AutoSetupSupabase] Falha na configuração de system_settings"
      );
    }
    ...
    if (allOk) {
      console.log(
        "✅ [AutoSetupSupabase] Configuração automática concluída com sucesso!"
      );
    } else {
      console.log(
        "⚠️ [AutoSetupSupabase] Configuração parcial - algumas tabelas precisam ser criadas manualmente"
      );
    }
    ...
    console.error(
      "❌ [AutoSetupSupabase] Erro na configuração automática:",
      error
    );
    return false;
  }

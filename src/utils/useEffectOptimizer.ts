...
const useEffectRegistry = new Map<
  string,
  {
    count: number;
    lastRun: number;
  ...
  useEffect(() => {
    ...
    const runEffect = () => {
      ...
      try {
        ...
        PerformanceAuditor.recordOperation(
          `useEffect_${effectId}`,
          duration,
          true
        );
        ...
        existing.averageTime =
          (existing.averageTime * (existing.count - 1) + duration) /
          existing.count;
        ...
        if (duration > 100) {
          console.warn(
            `🐌 [UseEffectOptimizer] Slow effect: ${effectId} (${duration.toFixed(2)}ms)`
          );
        }
      } catch (error) {
        const duration = performance.now() - startTime;
...
export function useSmartMemo<T>(
  ...
  const {
    name = "anonymous",
    maxAge = 60000,
    skipExpensiveCalculation = false,
  } = options;
  ...
  return useMemo(() => {
    ...
    if (lastCalculationRef.current && maxAge > 0) {
      ...
      const depsChanged =
        !deps ||
        !lastCalculationRef.current.deps ||
        deps.some(
          (dep, index) => dep !== lastCalculationRef.current!.deps[index]
        );
    ...
    if (duration > 50) {
      console.warn(
        `🐌 [SmartMemo] Expensive calculation: ${name} (${duration.toFixed(2)}ms)`
      );
    }
  ...
  return useCallback((...args: Parameters<T>) => {
    ...
    const executeCallback = () => {
      ...
      try {
        ...
        if (duration > 50) {
          console.warn(
            `🐌 [OptimizedCallback] Slow callback: ${name} (${duration.toFixed(2)}ms)`
          );
        }
...
export function useRenderTracker(
  ...
  useEffect(() => {
    console.log(
      `🔄 [RenderTracker] ${componentName} renderizado ${renderCountRef.current} vezes`
    );
    ...
    if (props && prevPropsRef.current) {
      ...
      if (changedProps.length > 0) {
        console.log(
          `📝 [RenderTracker] ${componentName} props alteradas:`,
          changedProps
        );
      } else {
        console.warn(
          `⚠️ [RenderTracker] ${componentName} re-renderizado sem mudança de props!`
        );
      }
    }
  ...
  if (renderCountRef.current > 10) {
    console.warn(
      `🚨 [RenderTracker] ${componentName} renderizado ${renderCountRef.current} vezes - possível problema de performance!`
    );
  }
}

...
export const optimizedSignIn = async (
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    console.error(
      `❌ [AuthOptimization] Login falhou após ${duration}ms:`,
      error
    );
    throw error;
  }
...
export const quickSessionCheck = async (): Promise<any> => {
  ...
  try {
    ...
    if (result && typeof result === "object" && "data" in result) {
      ...
      if (error) {
        console.warn(
          "⚠️ [AuthOptimization] Erro na verificação de sessão:",
          error
        );
        return null;
      }
    ...
    console.warn(
      "⚠️ [AuthOptimization] Timeout na verificação de sessão, usando fallback"
    );
    ...
    try {
      ...
      if (localSession) {
        ...
        if (parsed.expires_at && parsed.expires_at > Date.now() / 1000) {
          console.log(
            "✅ [AuthOptimization] Sessão válida encontrada no localStorage"
          );
          return parsed;
        }
      }
      ...
      console.warn(
        "⚠️ [AuthOptimization] Erro ao verificar localStorage:",
        localError
      );
    }
...
export const preloadCriticalData = async (): Promise<void> => {
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    console.log(
      `✅ [AuthOptimization] Dados críticos carregados em ${duration}ms`
    );
  } catch (error) {
    console.warn(
      "⚠️ [AuthOptimization] Erro no pré-carregamento (não crítico):",
      error
    );
  }
};
...
export const lazyLoadSecondaryData = async (): Promise<void> => {
  console.log(
    "⏳ [AuthOptimization] Carregamento lazy de dados secundários..."
  );
  ...
  try {
    ...
    console.warn(
      "⚠️ [AuthOptimization] Erro no carregamento secundário:",
      error
    );
  }
};
...
export class AuthPerformanceMonitor {
  ...
  static recordOperation(
    ...
    console.log(
      `📊 [AuthPerformance] ${operation}: ${duration}ms (${success ? "sucesso" : "falha"})`
    );
  }

  static getAverageTime(operation: string): number {
    const ops = this.metrics.filter(
...
export const initializeAuthOptimizations = (): void => {
  console.log(
    "🚀 [AuthOptimization] Inicializando otimizações de autenticação..."
  );

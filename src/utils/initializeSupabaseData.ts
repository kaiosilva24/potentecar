...
export async function checkSystemSettingsTable(): Promise<boolean> {
  try {
    ...
    if (error) {
      console.warn(
        "⚠️ [Checkpoint] Tabela system_settings não existe ou não acessível:",
        error.message
      );
      return false;
    }
    ...
    if (data && data.length > 0) {
      console.log(
        `📊 [Checkpoint] Encontrados ${data.length} registros na tabela`
      );
    } else {
      console.log(
        "📊 [Checkpoint] Tabela vazia - pronta para receber checkpoints"
      );
    }
...
export async function checkCheckpoint(
  key: string
): Promise<{ exists: boolean; value?: number }> {
  try {
    // Usar any para contornar restrições de TypeScript
    const { data, error } = await (supabase as any)
      .from("system_settings")
...
export async function ensureSystemDataExists(): Promise<void> {
  try {
    ...
    if (tableExists) {
      ...
      console.log(
        "💾 [Checkpoint] Os valores serão salvos automaticamente quando calculados"
      );
      console.log(
        "🔄 [Checkpoint] Ao reiniciar, os últimos valores calculados serão carregados"
      );
    } else {
      console.warn("⚠️ [Checkpoint] Sistema de checkpoint não está disponível");
      console.warn(
        "⚠️ [Checkpoint] Execute o SQL create_system_settings_table.sql no Supabase"
      );
    }
  } catch (error) {

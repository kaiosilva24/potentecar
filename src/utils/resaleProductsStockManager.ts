...
class ResaleProductsStockManager {
  ...
  loadStockData(): ResaleProductStockData[] {
    try {
      ...
      if (savedData) {
        ...
        console.log(
          `✅ [ResaleStockManager] Dados carregados: ${data.length} produtos`
        );
        return data;
      }
    } catch (error) {
  ...
  saveStockData(data: ResaleProductStockData[]): boolean {
    try {
      ...
      console.log(
        `✅ [ResaleStockManager] Dados salvos: ${data.length} produtos`
      );
      return true;
    } catch (error) {
   ...
   */
  updateProductStock(
    productData: Omit<ResaleProductStockData, "lastUpdated" | "createdAt">
  ): boolean {
    try {
      ...
      const fullProductData: ResaleProductStockData = {
        ...
        createdAt:
          existingIndex >= 0 ? currentData[existingIndex].createdAt : timestamp,
      };
      ...
      if (existingIndex >= 0) {
        ...
        console.log(
          `🔄 [ResaleStockManager] Produto atualizado: ${productData.productName}`
        );
      } else {
        // Adicionar novo produto
        currentData.push(fullProductData);
        console.log(
          `➕ [ResaleStockManager] Novo produto adicionado: ${productData.productName}`
        );
      }
      ...
      console.error(
        "❌ [ResaleStockManager] Erro ao atualizar produto:",
        error
      );
      return false;
    }
  ...
  getStockSummary(): ResaleProductStockSummary {
    try {
      ...
      const averageMargin =
        totalProducts > 0
          ? data.reduce((sum, item) => sum + item.profitMargin, 0) /
            totalProducts
          : 0;
  ...
  exportToFile(filename?: string): void {
    try {
      ...
      link.download =
        filename ||
        `estoque_produtos_revenda_${new Date().toISOString().split("T")[0]}.json`;
      ...
      console.log(
        `📥 [ResaleStockManager] Arquivo exportado: ${link.download}`
      );
    } catch (error) {
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          if (success) {
            console.log(
              `📤 [ResaleStockManager] Arquivo importado: ${validProducts.length} produtos`
            );
          }
          ...
          console.error(
            "❌ [ResaleStockManager] Erro ao importar arquivo:",
            error
          );
          resolve(false);
        }
...
export const loadResaleProductsStock = () =>
  resaleProductsStockManager.loadStockData();
export const saveResaleProductsStock = (data: ResaleProductStockData[]) =>
  resaleProductsStockManager.saveStockData(data);
export const updateResaleProductStock = (
  productData: Omit<ResaleProductStockData, "lastUpdated" | "createdAt">
) => resaleProductsStockManager.updateProductStock(productData);
export const getResaleProductStock = (productId: string) =>
  resaleProductsStockManager.getProductStock(productId);
export const getResaleStockSummary = () =>
  resaleProductsStockManager.getStockSummary();
export const exportResaleStockToFile = (filename?: string) =>
  resaleProductsStockManager.exportToFile(filename);
export const importResaleStockFromFile = (file: File) =>
  resaleProductsStockManager.importFromFile(file);
export const clearResaleStockData = () =>
  resaleProductsStockManager.clearAllData();

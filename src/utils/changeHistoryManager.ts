...
export class ChangeHistoryManager {
  ...
  async recordChange(
    ...
    try {
      ...
      if (error) {
        console.error(
          "❌ [ChangeHistoryManager] Erro ao salvar histórico:",
          error
        );
        throw error;
      }
      ...
      console.error(
        "❌ [ChangeHistoryManager] Erro ao registrar alteração:",
        error
      );
      throw error;
    }
  ...
  async undo(): Promise<boolean> {
    ...
    try {
      ...
      console.log(
        "🔄 [ChangeHistoryManager] Desfazendo alteração:",
        currentEntry
      );
      ...
      console.error(
        "❌ [ChangeHistoryManager] Erro ao desfazer alteração:",
        error
      );
      return false;
    }
  ...
  async redo(): Promise<boolean> {
    ...
    try {
      ...
      console.error(
        "❌ [ChangeHistoryManager] Erro ao refazer alteração:",
        error
      );
    ...
    try {
      switch (entry.operation_type) {
        ...
        case "UPDATE":
          ...
          if (!entry.old_data) {
            console.error(
              "❌ [ChangeHistoryManager] Dados antigos não encontrados para reversão"
            );
            return false;
          }
        ...
        case "DELETE":
          ...
          if (!entry.old_data) {
            console.error(
              "❌ [ChangeHistoryManager] Dados antigos não encontrados para recriação"
            );
            return false;
          }
        ...
        default:
          console.error(
            "❌ [ChangeHistoryManager] Tipo de operação desconhecido:",
      ...
      console.error(
        "❌ [ChangeHistoryManager] Erro ao reverter alteração:",
        error
      );
      return false;
    }
    ...
    try {
      switch (entry.operation_type) {
        case "CREATE":
          ...
          if (!entry.new_data) {
            console.error(
              "❌ [ChangeHistoryManager] Novos dados não encontrados para criação"
            );
            return false;
          }
        ...
        case "UPDATE":
          ...
          if (!entry.new_data) {
            console.error(
              "❌ [ChangeHistoryManager] Novos dados não encontrados para atualização"
            );
            return false;
          }
      ...
      console.error(
        "❌ [ChangeHistoryManager] Erro ao aplicar alteração:",
        error
      );
      return false;
    }
  ...
  async loadHistory(limit: number = 50): Promise<ChangeHistoryEntry[]> {
    try {
      ...
      if (error) {
        console.error(
          "❌ [ChangeHistoryManager] Erro ao carregar histórico:",
          error
        );
        throw error;
      }
      ...
      console.log(
        "✅ [ChangeHistoryManager] Histórico carregado:",
        data.length,
        "entradas"
      );
      return data;
  ...
  async getHistoryForDisplay(limit: number = 20): Promise<HistorySnapshot[]> {
    try {
      ...
      console.error(
        "❌ [ChangeHistoryManager] Erro ao formatar histórico:",
        error
      );
      return [];
    }
  ...
  async clearOldHistory(daysToKeep: number = 30): Promise<void> {
    try {
      ...
      if (error) {
        console.error(
          "❌ [ChangeHistoryManager] Erro ao limpar histórico antigo:",
          error
        );
        throw error;
      }
      ...
      console.error(
        "❌ [ChangeHistoryManager] Erro ao limpar histórico:",
        error
      );
    }
  }

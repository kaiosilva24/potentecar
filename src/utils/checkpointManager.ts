...
export class CheckpointManager {
  ...
  async createCheckpoint(source: string = "manual"): Promise<boolean> {
    try {
      ...
      const costOptionsStr = localStorage.getItem(
        "tireCostManager_costOptions"
      );
      const costOptions = costOptionsStr
        ? JSON.parse(costOptionsStr)
        : {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && key.startsWith("tireAnalysis_")) {
          try {
            ...
            console.warn(
              `⚠️ [CheckpointManager] Erro ao carregar ${key}:`,
              error
            );
          }
        }
      ...
      try {
        const syncDataStr = localStorage.getItem(
        ...
        console.warn(
          "⚠️ [CheckpointManager] Erro ao carregar dados sincronizados do TireCostManager:",
          error
        );
      }
  ...
  async restoreFromCheckpoint(): Promise<boolean> {
    try {
      console.log(
        "🔄 [CheckpointManager] Restaurando sistema do checkpoint..."
      );
      ...
      const restorePromises = [
        ...
        dataManager.saveFinalProductStockBalance(
          checkpoint.finalProductStockBalance
        ),
      ];
      ...
      localStorage.setItem(
        "tireCostManager_costOptions",
        JSON.stringify(checkpoint.costOptions)
      );
      ...
      if (checkpoint.tireAnalysisData) {
        Object.entries(checkpoint.tireAnalysisData).forEach(([key, data]) => {
          try {
            ...
            console.warn(
              `⚠️ [CheckpointManager] Erro ao restaurar ${key}:`,
              error
            );
          }
        });
      ...
      if (checkpoint.tireCostManagerData) {
        try {
          localStorage.setItem(
            "tireCostManager_synchronizedCostData",
            JSON.stringify(checkpoint.tireCostManagerData)
          );
          console.log(
            "✅ [CheckpointManager] Dados sincronizados do TireCostManager restaurados"
          );
        } catch (error) {
          console.warn(
            "⚠️ [CheckpointManager] Erro ao restaurar dados sincronizados do TireCostManager:",
            error
          );
        }
      }
      ...
      console.log(
        `✅ [CheckpointManager] Checkpoint restaurado com sucesso: ${successCount}/${results.length} valores`,
        {
          averageTireCost: `R$ ${checkpoint.averageTireCost.toFixed(2)}`,
          averageTireProfit: `R$ ${checkpoint.averageTireProfit.toFixed(2)}`,
          averageResaleProfit: `R$ ${checkpoint.averageResaleProfit.toFixed(2)}`,
          ...
          tireCostManagerData: checkpoint.tireCostManagerData
            ? "Restaurado"
            : "N/A",
        }
      );
      ...
      console.error(
        "❌ [CheckpointManager] Erro ao restaurar checkpoint:",
        error
      );
      return false;
    }
  ...
  private loadDashboardDataFromLocalStorage(): {
    ...
    try {
      ...
      const rawMaterialStr = localStorage.getItem(
        "dashboard_rawMaterialStockBalance"
      );
      const resaleProductStr = localStorage.getItem(
        "dashboard_resaleProductStockBalance"
      );

      const cashBalance = cashBalanceStr
        ? JSON.parse(cashBalanceStr).value || 0
        : 0;
      const totalRevenue = totalRevenueStr
        ? JSON.parse(totalRevenueStr).value || 0
        : 0;
      const rawMaterialStockBalance = rawMaterialStr
        ? JSON.parse(rawMaterialStr).value || 0
        : 0;
      const resaleProductStockBalance = resaleProductStr
        ? JSON.parse(resaleProductStr).value || 0
        : 0;
      ...
      console.error(
        "❌ [CheckpointManager] Erro ao carregar dados do dashboard:",
        error
      );
      return {
        cashBalance: 0,
  ...
  private saveDashboardDataToLocalStorage(dashboardData: {
    ...
    try {
      ...
      console.log(
        "💾 [CheckpointManager] Dados do dashboard salvos no localStorage"
      );
    } catch (error) {
      console.error(
        "❌ [CheckpointManager] Erro ao salvar dados do dashboard:",
        error
      );
    }
  }
  ...
  private loadCheckpointFromLocalStorage(): SystemCheckpoint | null {
    try {
      ...
      if (!checkpoint.timestamp || !checkpoint.version) {
        console.warn(
          "⚠️ [CheckpointManager] Checkpoint inválido no localStorage"
        );
        return null;
      }

      console.log(
        "📱 [CheckpointManager] Checkpoint carregado do localStorage"
      );
      return checkpoint;
    } catch (error) {
      console.error(
        "❌ [CheckpointManager] Erro ao carregar checkpoint do localStorage:",
        error
      );
      return null;
    }
   ...
   */
  private async saveCheckpointToSupabase(
    checkpoint: SystemCheckpoint
  ): Promise<boolean> {
    try {
      const success = await dataManager.saveSystemSetting(
        "system_checkpoint",
      ...
      console.error(
        "❌ [CheckpointManager] Erro ao salvar checkpoint no Supabase:",
        error
      );
      return false;
    }
  ...
  private async loadCheckpointFromSupabase(): Promise<SystemCheckpoint | null> {
    try {
      const checkpointStr =
        await dataManager.loadSystemSetting("system_checkpoint");
      if (!checkpointStr) return null;
      ...
      console.error(
        "❌ [CheckpointManager] Erro ao carregar checkpoint do Supabase:",
        error
      );
      return null;
    }
  ...
  private dispatchRestoreEvents(checkpoint: SystemCheckpoint): void {
    ...
    window.dispatchEvent(
      new CustomEvent("systemRestored", {
        detail: {
    ...
    window.dispatchEvent(
      new CustomEvent("cashBalanceUpdated", {
        detail: {
          balance: checkpoint.dashboardData.cashBalance,
          source: "CheckpointRestore",
        },
      })
    );
    ...
    window.dispatchEvent(
      new CustomEvent("totalRevenueUpdated", {
        detail: {
          revenue: checkpoint.dashboardData.totalRevenue,
          source: "CheckpointRestore",
        },
      })
    );
    ...
    window.dispatchEvent(
      new CustomEvent("finalProductStockUpdated", {
        detail: {
          balance: checkpoint.finalProductStockBalance,
          source: "CheckpointRestore",
        },
      })
    );
    ...
    window.dispatchEvent(
      new CustomEvent("tireCostUpdated", {
        detail: {
          averageCostPerTire: checkpoint.averageTireCost,
          source: "CheckpointRestore",
        },
      })
    );
    ...
    window.dispatchEvent(
      new CustomEvent("tireProfitUpdated", {
        detail: {
          profit: checkpoint.averageTireProfit,
          source: "CheckpointRestore",
        },
      })
    );
    ...
    window.dispatchEvent(
      new CustomEvent("resaleProfitUpdated", {
        detail: {
          profit: checkpoint.averageResaleProfit,
          source: "CheckpointRestore",
        },
      })
    );
    ...
    window.dispatchEvent(
      new CustomEvent("resaleProductStockUpdated", {
        detail: {
          balance: checkpoint.dashboardData.resaleProductStockBalance,
          source: "CheckpointRestore",
        },
      })
    );
    ...
    window.dispatchEvent(
      new CustomEvent("rawMaterialStockUpdated", {
        detail: {
          balance: checkpoint.dashboardData.rawMaterialStockBalance,
          source: "CheckpointRestore",
        },
      })
    );
    ...
    if (
      checkpoint.tireAnalysisData &&
      Object.keys(checkpoint.tireAnalysisData).length > 0
    ) {
      window.dispatchEvent(
        new CustomEvent("tireAnalysisDataRestored", {
          detail: {
    ...
    if (checkpoint.tireCostManagerData) {
      window.dispatchEvent(
        new CustomEvent("tireCostManagerDataRestored", {
          detail: {
    ...
    window.dispatchEvent(
      new CustomEvent("forceCostDataUpdate", {
        detail: {
          timestamp: Date.now(),
          source: "CheckpointRestore",
        },
  ...
  clearCheckpoint(): boolean {
    try {
      ...
      console.error(
        "❌ [CheckpointManager] Erro ao remover checkpoint:",
        error
      );
      return false;
    }

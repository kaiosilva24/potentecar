...
export class DataManagerWithHistory {
  ...
  >(tableName: string, data: T, description?: string): Promise<T | null> {
    try {
      ...
      if (isUpdate) {
        try {
          ...
          console.warn(
            "⚠️ [DataManagerWithHistory] Não foi possível buscar dados antigos:",
            error
          );
        }
      }
      ...
      if (result) {
        ...
        console.log(
          `✅ [DataManagerWithHistory] ${operation} registrado no histórico:`,
          {
            tableName,
            recordId,
      ...
      console.error(
        "❌ [DataManagerWithHistory] Erro ao salvar com histórico:",
        error
      );
      throw error;
    }
  ...
  async deleteFromDatabase(
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "⚠️ [DataManagerWithHistory] Não foi possível buscar dados antes da exclusão:",
          error
        );
      }
      ...
      if (error) {
        console.error(
          `❌ [DataManagerWithHistory] Erro ao deletar de ${tableName}:`,
          error
        );
        return false;
      }
      ...
      console.log(
        `✅ [DataManagerWithHistory] DELETE registrado no histórico:`,
        {
          tableName,
          recordId: id,
          description: description || autoDescription,
      ...
      console.error(
        "❌ [DataManagerWithHistory] Erro ao deletar com histórico:",
        error
      );
      return false;
    }
  ...
  async loadServices() {
    return dataManager.loadServices();
  }
  async loadTransactions() {
    return dataManager.loadTransactions();
  }
  async loadStockItems() {
    return dataManager.loadStockItems();
  }
  async loadCashFlowEntries() {
    return dataManager.loadCashFlowEntries();
  }
  async loadExpenses() {
    return dataManager.loadExpenses();
  }
  async loadDebts() {
    return dataManager.loadDebts();
  }
  async loadResaleProducts() {
    return dataManager.loadResaleProducts();
  }
  async loadBusinessValue() {
    return dataManager.loadBusinessValue();
  }
  async saveBusinessValue(value: number) {
    return dataManager.saveBusinessValue(value);
  }
  async loadAverageTireCost() {
    return dataManager.loadAverageTireCost();
  }
  async saveAverageTireCost(cost: number) {
    return dataManager.saveAverageTireCost(cost);
  }
  async loadSystemSetting(key: string) {
    return dataManager.loadSystemSetting(key);
  }
  async saveSystemSetting(key: string, value: any) {
    return dataManager.saveSystemSetting(key, value);
  }
  async exportDatabase() {
    return dataManager.exportDatabase();
  }
  async importDatabase(data: string) {
    return dataManager.importDatabase(data);
  }
  generateBackupFileName() {
    return dataManager.generateBackupFileName();
  }

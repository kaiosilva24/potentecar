...
export class ProductionProfitManager {
  ...
  static async addProductionProfitToBaseline(
    ...
    try {
      console.log(
        "🏭 [ProductionProfitManager] Calculando diferença de produção para adicionar ao baseline..."
      );
      ...
      console.log(
        `💰 [ProductionProfitManager] Custo por pneu (receita) para "${productName}": R$ ${costPerTire.toFixed(2)}`
      );
      ...
      for (const material of materialsConsumed) {
        ...
        if (stockItem) {
          ...
          console.log(
            `📦 [ProductionProfitManager] Material "${material.material_name}": ${material.quantity_consumed} x R$ ${stockItem.unit_cost.toFixed(2)} = R$ ${materialCost.toFixed(2)}`
          );
        } else {
          console.warn(
            `⚠️ [ProductionProfitManager] Material não encontrado no estoque: ${material.material_name}`
          );
        }
      }
      ...
      const materialCostPerUnit =
        quantityProduced > 0 ? totalMaterialCost / quantityProduced : 0;
      console.log(
        `📊 [ProductionProfitManager] Custo total dos materiais: R$ ${totalMaterialCost.toFixed(2)}`
      );
      console.log(
        `📊 [ProductionProfitManager] Custo por unidade dos materiais: R$ ${materialCostPerUnit.toFixed(2)}`
      );
      ...
      if (totalProfit > 0) {
        ...
        if (currentBaseline !== null) {
          ...
          const success =
            await dataManager.saveBusinessValueBaseline(newBaseline);

          if (success) {
            console.log(
              `✅ [ProductionProfitManager] Baseline atualizado com sucesso!`
            );
            console.log(
            ...
            alert(
              `🎉 LUCRO DA PRODUÇÃO ADICIONADO AO BASELINE!\n\n` +
                `📊 ANÁLISE DA PRODUÇÃO:\n` +
                `• Produto: ${productName}\n` +
            ...
            console.error(
              "❌ [ProductionProfitManager] Falha ao salvar novo baseline"
            );
            return false;
          }
        } else {
          console.warn(
            "⚠️ [ProductionProfitManager] Baseline não definido, não é possível adicionar lucro da produção"
          );
          console.log(
            "💡 [ProductionProfitManager] Confirme o balanço empresarial primeiro para habilitar esta funcionalidade"
          );

          alert(
            `⚠️ BASELINE NÃO DEFINIDO\n\n` +
        ...
        console.log(
          `⚠️ [ProductionProfitManager] Produção com prejuízo detectada (R$ ${totalProfit.toFixed(2)})`
        );
        console.log(
          `💡 [ProductionProfitManager] Não será adicionado ao baseline pois é prejuízo`
        );

        alert(
          `⚠️ PRODUÇÃO COM PREJUÍZO DETECTADA!\n\n` +
            `📊 ANÁLISE DA PRODUÇÃO:\n` +
            `• Produto: ${productName}\n` +
            `• Quantidade: ${quantityProduced} unidades\n` +
            `• Custo/Pneu (Receita): R$ ${costPerTire.toFixed(2)}\n` +
        ...
        console.log(
          `ℹ️ [ProductionProfitManager] Produção sem lucro nem prejuízo (diferença: R$ ${totalProfit.toFixed(2)})`
        );
        return true;
      }
    } catch (error) {
      console.error(
        "❌ [ProductionProfitManager] Erro ao adicionar lucro da produção ao baseline:",
        error
      );
      alert(
        `❌ ERRO ao processar lucro da produção:\n\n${error}\n\nVerifique o console para mais detalhes.`
      );
      return false;
    }
  }
   ...
   */
  private static async getSpecificProductCost(
    productName: string
  ): Promise<number> {
    try {
      ...
      if (savedAnalysis) {
        try {
          ...
          if (analysis.costPerTire && analysis.costPerTire > 0) {
            console.log(
              `✅ [ProductionProfitManager] Custo específico encontrado para "${productName}": R$ ${analysis.costPerTire.toFixed(2)}`
            );
            return analysis.costPerTire;
          }
        } catch (parseError) {
          console.warn(
            `⚠️ [ProductionProfitManager] Erro ao parsear análise específica para "${productName}":`,
            parseError
          );
        }
      }
      ...
      console.log(
        `📊 [ProductionProfitManager] Usando custo médio para "${productName}": R$ ${averageCost.toFixed(2)}`
      );
      return averageCost;
    } catch (error) {
      console.error(
        `❌ [ProductionProfitManager] Erro ao buscar custo específico para "${productName}":`,
        error
      );
      return 0;
    }
  ...
  static async removeProductionProfitFromBaseline(
    ...
    try {
      console.log(
        "🔄 [ProductionProfitManager] Removendo lucro da produção do baseline..."
      );
      console.log(
        "📊 [ProductionProfitManager] Parâmetros recebidos para reversão:",
        {
          productName,
          quantityProduced,
          materialsCount: materialsConsumed.length,
      ...
      console.log(`   • Quantidade produzida: ${quantityProduced}`);
      console.log(
        `   • Lucro total a remover: R$ ${totalProfitToRemove.toFixed(2)}`
      );
      ...
      if (totalProfitToRemove > 0) {
        ...
        if (currentBaseline !== null) {
          ...
          if (success) {
            console.log(
              `✅ [ProductionProfitManager] Baseline atualizado com sucesso após exclusão!`
            );
            console.log(
              `   • Baseline anterior: R$ ${currentBaseline.toFixed(2)}`
            );
            console.log(
              `   • Lucro removido: R$ ${totalProfitToRemove.toFixed(2)}`
            );
            console.log(`   • Novo baseline: R$ ${newBaseline.toFixed(2)}`);
            ...
            // Mostrar alerta de sucesso para o usuário
            alert(
              `🔄 LUCRO DA PRODUÇÃO REMOVIDO DO BASELINE!\n\n` +
                `📊 PRODUÇÃO EXCLUÍDA:\n` +
                `• Produto: ${productName}\n` +
                `• Quantidade: ${quantityProduced} unidades\n` +
                `• Custo/Pneu (Receita): R$ ${costPerTire.toFixed(2)}\n` +
            ...
            console.error(
              "❌ [ProductionProfitManager] Falha ao salvar novo baseline após exclusão"
            );
            return false;
          }
        } else {
          console.warn(
            "⚠️ [ProductionProfitManager] Baseline não definido, não é possível remover lucro da produção"
          );
          return false;
        }
      } else if (totalProfitToRemove < 0) {
        console.log(
          `⚠️ [ProductionProfitManager] Produção excluída tinha prejuízo (R$ ${totalProfitToRemove.toFixed(2)})`
        );
        console.log(
          `💡 [ProductionProfitManager] Não há lucro para remover do baseline`
        );
        return true;
      } else {
        console.log(
          `ℹ️ [ProductionProfitManager] Produção excluída não tinha lucro nem prejuízo`
        );
        return true;
      }
    } catch (error) {
      console.error(
        "❌ [ProductionProfitManager] Erro ao remover lucro da produção do baseline:",
        error
      );
      alert(
        `❌ ERRO ao processar remoção do lucro da produção:\n\n${error}\n\nVerifique o console para mais detalhes.`
      );
      return false;
    }
  }
  ...
  static async isBaselineDefined(): Promise<boolean> {
    try {
      ...
      console.error(
        "❌ [ProductionProfitManager] Erro ao verificar baseline:",
        error
      );
      return false;
    }
  ...
  static async getBaselineInfo(): Promise<{
    ...
    try {
      ...
      console.error(
        "❌ [ProductionProfitManager] Erro ao obter informações do baseline:",
        error
      );
      return {
        baseline: null,

...
export class DataManager {
  ...
  async deleteProduct(id: string): Promise<boolean> {
    try {
      console.log(` [DataManager] Deletando produto permanentemente: ${id}`);

      // First, remove any related stock items
      console.log(
        ` [DataManager] Removendo itens de estoque relacionados ao produto: ${id}`,
      );
      const { error: stockError } = await this.supabase
        .from("stock_items")
        .delete()
      ...
      if (stockError) {
        console.warn(
          " [DataManager] Erro ao remover itens de estoque (pode não existir):",
          stockError,
        );
  ...
  // Delete resale product permanently
  async deleteResaleProduct(id: string): Promise<boolean> {
    try {
      console.log(` [DataManager] Deletando produto de revenda permanentemente: ${id}`);

      // First, remove any related stock items
      console.log(
        ` [DataManager] Removendo itens de estoque relacionados ao produto de revenda: ${id}`,
      );
      const { error: stockError } = await this.supabase
        .from("stock_items")
        .delete()
      ...
      if (stockError) {
        ...
        // Continue with resale product deletion even if stock removal fails
      } else {
        console.log(" [DataManager] Itens de estoque removidos com sucesso");
      }

      // Then delete the resale product
      const { error: productError } = await this.supabase
      ...
      if (productError) {
        console.error(
          " [DataManager] Erro ao deletar produto de revenda:",
          productError,
        );
        return false;
      }
      ...
      console.log(
        " [DataManager] Produto de revenda deletado permanentemente com sucesso",
      );
      return true;
    } catch (error) {
      console.error(" [DataManager] Erro crítico ao deletar produto de revenda:", error);
      return false;
    }
  }

...
export class CheckpointIntegration {
  ...
  async initialize(): Promise<void> {
    try {
      console.log(
        "🚀 [CheckpointIntegration] Inicializando sistema de checkpoint..."
      );
      ...
      console.log(
        "✅ [CheckpointIntegration] Sistema de checkpoint inicializado com sucesso"
      );
    } catch (error) {
      console.error(
        "❌ [CheckpointIntegration] Erro ao inicializar sistema de checkpoint:",
        error
      );
    }
  }
  ...
  private async restoreFromLastCheckpoint(): Promise<void> {
    ...
    if (checkpointInfo.exists) {
      console.log(
        `📥 [CheckpointIntegration] Checkpoint encontrado (${checkpointInfo.age})`
      );
      ...
      if (shouldRestore) {
        ...
        console.log(
          "✅ [CheckpointIntegration] Estado restaurado do checkpoint"
        );
      }
    } else {
    ...
    this.autoSaveInterval = setInterval(
      async () => {
        try {
          ...
          console.log(
            "💾 [CheckpointIntegration] Checkpoint automático criado"
          );
        } catch (error) {
          console.error(
            "❌ [CheckpointIntegration] Erro no checkpoint automático:",
            error
          );
        }
      },
  ...
  private setupEventListeners(): void {
    ...
    const handleImportantEvent = () => {
      ...
      debounceTimer = setTimeout(async () => {
        try {
          await this.checkpointManager.createCheckpoint();
          console.log(
            "💾 [CheckpointIntegration] Checkpoint criado por evento importante"
          );
        } catch (error) {
          console.error(
            "❌ [CheckpointIntegration] Erro ao criar checkpoint por evento:",
            error
          );
        }
    ...
    window.addEventListener("beforeunload", async () => {
      try {
        await this.checkpointManager.createCheckpoint();
        console.log(
          "💾 [CheckpointIntegration] Checkpoint criado antes de fechar"
        );
      } catch (error) {
        console.error(
          "❌ [CheckpointIntegration] Erro ao criar checkpoint final:",
          error
        );
      }
    });
  ...
  async createManualCheckpoint(): Promise<void> {
    try {
      ...
      console.error(
        "❌ [CheckpointIntegration] Erro ao criar checkpoint manual:",
        error
      );
      throw error;
    }
  ...
  async restoreManually(): Promise<void> {
    try {
      ...
      console.error(
        "❌ [CheckpointIntegration] Erro na restauração manual:",
        error
      );
      throw error;
    }

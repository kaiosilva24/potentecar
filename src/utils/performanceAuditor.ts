...
export class PerformanceAuditor {
  ...
  static endMeasurement(
    ...
    if (metric) {
      ...
      const color =
        metric.duration > 5000 ? "🔴" : metric.duration > 2000 ? "🟡" : "🟢";

      console.log(
        `${status} [PerformanceAuditor] ${color} ${operation}: ${metric.duration.toFixed(2)}ms`,
        {
          success,
          error,
      ...
      if (metric.duration > 10000) {
        console.warn(
          `🚨 [PerformanceAuditor] OPERAÇÃO MUITO LENTA: ${operation} (${metric.duration.toFixed(2)}ms)`
        );
      }
    }
  }
  ...
  static async analyzeQuery<T>(
    ...
    try {
      ...
      const analysis: QueryAnalysis = {
        ...
        rowCount:
          result?.data?.length ||
          (result?.count !== undefined ? result.count : undefined),
      };
      ...
      console.log(
        `${color} [QueryAnalyzer] ${table}.${operation}: ${duration.toFixed(2)}ms`,
        {
      ...
      if (duration > 5000) {
        console.warn(
          `🐌 [QueryAnalyzer] Query lenta detectada: ${table}.${operation} (${duration.toFixed(2)}ms)`
        );
      }
  ...
  static async runSystemAudit(): Promise<{
    ...
    console.log(
      "🔍 [PerformanceAuditor] Iniciando auditoria completa do sistema..."
    );
    ...
    if (slowOperations.length > 0) {
      recommendations.push(
        `🐌 ${slowOperations.length} operações lentas detectadas (>2s)`
      );

      const slowestOp = slowOperations[0];
      if (slowestOp.duration && slowestOp.duration > 10000) {
        recommendations.push(
          `🚨 Operação crítica: ${slowestOp.operation} (${slowestOp.duration.toFixed(2)}ms)`
        );
      }
    }
    ...
    if (slowQueries.length > 0) {
      recommendations.push(
        `🗄️ ${slowQueries.length} queries lentas detectadas (>1s)`
      );

      const slowestQuery = slowQueries[0];
      if (slowestQuery.duration > 5000) {
        recommendations.push(
          `🚨 Query crítica: ${slowestQuery.table}.${slowestQuery.operation} (${slowestQuery.duration.toFixed(2)}ms)`
        );
      }
    }
    ...
    if (frequentOps.length > 0) {
      recommendations.push(
        `🔄 Operações muito frequentes: ${frequentOps.map(([op, count]) => `${op}(${count}x)`).join(", ")}`
      );
    }
    ...
    const summary = {
      ...
      averageOperationTime:
        ...
        this.metrics.length,
      averageQueryTime:
        this.queryAnalytics.reduce((sum, q) => sum + q.duration, 0) /
        this.queryAnalytics.length,
      successRate:
  ...
  static async auditDashboardLoad(): Promise<void> {
    console.log(
      "🏠 [PerformanceAuditor] Auditando carregamento do dashboard..."
    );
    ...
    try {
      ...
      await Promise.all([
        ...
        this.analyzeQuery(
          "stock_items",
          "dashboard_stock",
          async () => await supabase.from("stock_items").select("*").limit(50)
        ),
        this.analyzeQuery(
          "system_settings",
          "dashboard_settings",
          async () => await supabase.from("system_settings").select("*")
        ),
      ]);
  ...
  static auditDataHooks(): void {
    ...
    React.useEffect = (...args) => {
      ...
      if (useEffectCount > 50) {
        console.warn(
          `⚠️ [PerformanceAuditor] Muitos useEffects detectados: ${useEffectCount}`
        );
      }
  ...
  static clearOldMetrics(): void {
    ...
    this.queryAnalytics = this.queryAnalytics.filter(
      (q) => q.duration > oneHourAgo
    );
  ...
  static getPerformanceReport(): string {
    ...
    const slowQueries = this.queryAnalytics.filter(
      (q) => q.duration > 1000
    ).length;
...
${failedOps > 0 ? "• Investigar falhas recorrentes" : "✅ Taxa de sucesso boa"}
  ...
  static setEnabled(enabled: boolean): void {
    ...
    console.log(
      `${enabled ? "🟢" : "🔴"} [PerformanceAuditor] Auditoria ${enabled ? "habilitada" : "desabilitada"}`
    );
  }
...
export function withPerformanceMonitoring<T extends (...args: any[]) => any>(
  ...
  return ((...args: any[]) => {
    const id = PerformanceAuditor.startMeasurement(operationName, {
      args: args.length,
    });
    ...
    try {
      ...
      if (result && typeof result.then === "function") {
        return result
          ...
          .catch((error: any) => {
            PerformanceAuditor.endMeasurement(
              operationName,
              false,
              error?.message
            );
